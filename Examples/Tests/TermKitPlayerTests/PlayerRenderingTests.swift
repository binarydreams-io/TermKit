...
struct PlayerRenderingTests {
    @Test("Player decodes and renders its original PNG and JPEG artwork")
    func artwork() throws {
    ...
    @Test("Paused player is idle and playing player requests timeline frames")
    func frameDemand() throws {
        let artwork = try loadPlayerArtwork()
    ...
    @Test("Spectrum uses cyan data cells and red peak cells")
    func spectrumPeakStyles() throws {
    ...
    @Test(
        "Canonical player layouts match semantic snapshots",
        arguments: [
            (40, 16), (80, 24), (120, 32), (180, 40),
        ]
    )
    func semanticSnapshots(width: Int, height: Int) throws {
        let model = PlayerModel(timeSource: DeterministicTimeSource())
        ...
        let snapshot = playerSnapshot(
            surface: try #require(output.presenter.frontSurface),
...
private final class PlayerTestSession: RuntimeTerminalSession {
    ...
    func readTerminalSize() throws -> TerminalSize { TerminalSize(columns: 1, rows: 1) }
}
