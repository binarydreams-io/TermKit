...
import Testing
    ...
    struct TermKitPlayerSmokeTests {
        ...
        @Test("Player starts, handles input and resize, and restores a PTY", .timeLimit(.minutes(1)))
        func playerPTYSmoke() throws {
            ...
            var output = try pty.readUntil(operation: "waiting for full player frame", timeoutMilliseconds: 10_000) { bytes in
                guard let text = String(bytes: bytes, encoding: .utf8) else { return false }
                return text.contains("FULL DECK // TK-97")
            }

            try pty.writeToMaster(Array(" ".utf8), timeoutMilliseconds: 1_000)
            output += try pty.readUntil(operation: "waiting for play input frame", timeoutMilliseconds: 5_000) { bytes in
                bytes.isEmpty == false
            }
            try pty.writeToMaster(Array("m".utf8), timeoutMilliseconds: 1_000)
            output += try pty.readUntil(operation: "waiting for mute input frame", timeoutMilliseconds: 5_000) { bytes in
                bytes.isEmpty == false
            }
            try pty.writeToMaster(Array(" ".utf8), timeoutMilliseconds: 1_000)
            output += try pty.readUntil(operation: "waiting for pause input frame", timeoutMilliseconds: 5_000) { bytes in
                bytes.isEmpty == false
            }
            try pty.resize(columns: 56, rows: 18)
            guard kill(processID, SIGWINCH) == 0 else { throw SmokeError.posix("kill(SIGWINCH)", errno) }
            output += try pty.readUntil(operation: "waiting for compact player frame", timeoutMilliseconds: 5_000) { bytes in
                bytes.isEmpty == false
            }
            try pty.resize(columns: 40, rows: 16)
            guard kill(processID, SIGWINCH) == 0 else { throw SmokeError.posix("kill(SIGWINCH)", errno) }
            output += try pty.readUntil(operation: "waiting for minimum player frame", timeoutMilliseconds: 5_000) { bytes in
                bytes.isEmpty == false
            }
            try pty.writeToMaster(Array("q".utf8), timeoutMilliseconds: 1_000)
            output += try pty.readUntil(operation: "waiting for terminal restoration", timeoutMilliseconds: 5_000) { bytes in
