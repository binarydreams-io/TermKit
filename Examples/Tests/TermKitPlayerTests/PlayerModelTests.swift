...
struct PlayerModelTests {
  @Test
  func `Playback clock advances only while playing and clamps at duration`() {
  ...
  @Test
  func `Transport, seek, volume, and playback modes are deterministic`() {
    let model = PlayerModel(timeSource: DeterministicTimeSource())
  ...
  @Test
  func `Queue selection and activation keep selected and current tracks distinct`() {
    let model = PlayerModel(timeSource: DeterministicTimeSource())
  ...
  @Test
  func `Application command set maps the complete global shortcut table`() {
    let model = PlayerModel(timeSource: DeterministicTimeSource())
  ...
  func `Responsive mode requires both width and height thresholds`(size: CellSize, expected: PlayerLayoutMode) {
