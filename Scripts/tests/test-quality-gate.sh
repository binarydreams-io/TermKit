...
prepare_project() {
    ...
    cp "$FIXTURES_DIR/scripts/test-api-snapshot-coverage.sh" "$test_root/scripts/tests/test-api-snapshot-coverage.sh"
    cp "$FIXTURES_DIR/scripts/validate-api-snapshot-coverage.sh" "$test_root/scripts/validate-api-snapshot-coverage.sh"
    cp "$FIXTURES_DIR/scripts/validate-benchmark-matrix.sh" "$test_root/scripts/validate-benchmark-matrix.sh"
    cp "$FIXTURES_DIR/scripts/swifttui-fuzz.sh" "$test_root/scripts/swifttui-fuzz.sh"
    ...
    chmod +x "$test_root/bin/"* "$test_root/scripts/"*.sh "$test_root/scripts/tests/"*.sh
...
test_runs_every_gate_in_order() {
    ...
    assert_log_line "$test_root/commands.log" 5 "test-api-snapshot-coverage"
    assert_log_line "$test_root/commands.log" 6 "validate-api-snapshot-coverage"
    assert_log_line "$test_root/commands.log" 7 "validate-benchmark-matrix"
    assert_log_line "$test_root/commands.log" 8 "validate-test-boundaries"
    assert_log_line "$test_root/commands.log" 9 "swiftlint lint --strict --no-cache"
    ...
    assert_log_line "$test_root/commands.log" 13 \
        "swifttui-fuzz --package-path $test_root --build-path $test_root/build"
    ...
    assert_log_line "$test_root/commands.log" 15 "verify-versioned-consumer "
    ...
    assert_log_line "$test_root/commands.log" 17 \
    ...
    assert_log_line "$test_root/commands.log" 18 \
        "swift test list --package-path $test_root --build-path $test_root/build --skip-build"
    assert_log_line "$test_root/commands.log" 19 "generate-documentation $test_root/docc-output"
    assert_log_line "$test_root/commands.log" 20 \
        "update-test-count --count-only --test-list $test_root/test-list.txt --expected-test-target TUIkitCoreTests --expected-test-target TUIkitStylingTests --expected-test-target TUIkitViewTests --expected-test-target TUIkitImageTests --expected-test-target TUIkitTests --expected-test-target TUIFoundationTests --expected-test-target TUITerminalTests --expected-test-target TUIRendererTests --expected-test-target TUIViewGraphTests --expected-test-target TUILayoutTests --expected-test-target TUIAnimationTests --expected-test-target TUIControlsTests --expected-test-target TUIDesignTests --expected-test-target TUIRichTextTests --expected-test-target TUIAgentUITests --expected-test-target TUIRuntimeTests --expected-test-target SwiftTUIIntegrationTests --expected-test-target SwiftTUIShowcaseSmokeTests --expected-test-target TUIPerformanceTests"
    [[ "$(wc -l < "$test_root/commands.log" | tr -d '[:space:]')" == "20" ]] || {
