...
case " $* " in
    ...
    *)
        ...
        mkdir -p "$module_path"
        for module in SwiftTUI TUIAgentUI TUIAnimation TUIControls TUIDesign TUIFoundation TUILayout \
            TUIRenderer TUIRichText TUIRuntime TUITerminal TUIViewGraph \
            TUIkit TUIkitCore TUIkitImage TUIkitStyling TUIkitView; do
...
test_tuikit_orchestrator_generates_host_sources() {
    ...
    [[ "$(find "$linux_output/sources" -type f -name '*.tsv' | wc -l | tr -d '[:space:]')" == "17" ]] || {
        fail "Linux orchestrator did not create all 17 source records"
    }
    local linux_record
    ...
    [[ "$(find "$macos_output/sources" -type f -name '*.tsv' | wc -l | tr -d '[:space:]')" == "17" ]] || {
        fail "macOS orchestrator did not create all 17 source records"
    }
    local macos_record
...
test_tuikit_orchestrator_supports_system_bash_without_optional_arguments() {
    ...
    [[ "$(find "$output_root/sources" -type f -name '*.tsv' | wc -l | tr -d '[:space:]')" == "17" ]] || {
        fail "system Bash did not generate all 17 Linux source records"
    }
}
