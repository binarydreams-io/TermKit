...
test_ci_rejects_duplicate_badge_writer() {
    ...
    expect_failure "CI configuration error: .github/workflows/update-test-counts.yml invokes the retired README test-count writer" \
...
test_ci_rejects_direct_readme_writer() {
    ...
    expect_failure "CI configuration error: .github/workflows/direct-readme-writer.yml contains a retired README test-count writer" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_contents_write_outside_badge_job() {
    ...
    expect_failure "CI configuration error: .github/workflows/contents-write-workflow.yml grants contents: write" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_write_all_outside_badge_job() {
    ...
    expect_failure "CI configuration error: .github/workflows/write-all-workflow.yml grants contents: write" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_quoted_contents_write_outside_badge_job() {
    ...
    expect_failure "CI configuration error: .github/workflows/quoted-contents-write-workflow.yml grants contents: write" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_indirect_badge_writer() {
    ...
    expect_failure "CI configuration error: .github/workflows/indirect-badge-writer.yml contains git push" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_git_global_option_push() {
    ...
    expect_failure "CI configuration error: .github/workflows/git-global-option-push-workflow.yml contains git push" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_git_flag_push() {
    ...
    expect_failure "CI configuration error: .github/workflows/git-flag-push-workflow.yml contains git push" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_git_line_continuation_push() {
    ...
    expect_failure "CI configuration error: .github/workflows/git-line-continuation-push-workflow.yml contains git push" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_requires_scoped_badge_permissions() {
    ...
    cp "$FIXTURES_DIR/ci/ci-without-badge-permissions.yml" \
        "$invalid_root/.github/workflows/ci.yml"
    expect_failure "CI configuration error: .github/workflows/ci.yml invokes the retired README test-count writer" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_ci_rejects_missing_binary_hashes() {
    ...
    chmod u+w "$invalid_hash_root/scripts/toolchain.env"
...
test_ci_requires_performance_xcode_selection() {
    local invalid_root="$TEMP_DIR/performance xcode fixture"
    cp -R "$FIXTURES_DIR/ci/valid" "$invalid_root"
    perl -0pi -e 's/^          sudo xcode-select --switch "\$SWIFT_MACOS_XCODE_PATH"\n//m' \
        "$invalid_root/.github/workflows/ci.yml"
    expect_failure "CI configuration error: performance CI must select the pinned macOS Swift toolchain" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}

test_ci_requires_performance_swift_version_assertion() {
    local invalid_root="$TEMP_DIR/performance version fixture"
    cp -R "$FIXTURES_DIR/ci/valid" "$invalid_root"
    perl -0pi -e 's/^          swift --version \| grep -F "Swift version \$SWIFT_VERSION"\n//m' \
        "$invalid_root/.github/workflows/ci.yml"
    expect_failure "CI configuration error: performance CI must assert the pinned Swift version" \
        "$PROJECT_DIR/scripts/validate-ci-configuration.sh" "$invalid_root"
}
...
test_benchmark_preserves_optional_arguments() {
    local fixture_root="$TEMP_DIR/benchmark fixture"
    local bin_dir="$fixture_root/bin"
    local log_file="$fixture_root/swift.log"
    mkdir -p "$bin_dir"
    cp "$FIXTURES_DIR/benchmark/swift" "$bin_dir/swift"
    chmod +x "$bin_dir/swift"

    PATH="$bin_dir:$PATH" BENCHMARK_SWIFT_LOG="$log_file" \
        "$PROJECT_DIR/scripts/swifttui-benchmark.sh" > /dev/null
    assert_contains "$log_file" "command: test"
    assert_contains "$log_file" "argument: list"
    assert_contains "$log_file" "argument: TUIPerformanceTests"

    : > "$log_file"
    PATH="$bin_dir:$PATH" BENCHMARK_SWIFT_LOG="$log_file" \
        "$PROJECT_DIR/scripts/swifttui-benchmark.sh" \
        --scratch-path "$fixture_root/build output" > /dev/null
    [[ "$(grep -Fc "argument: --scratch-path" "$log_file")" == "2" ]] \
        || fail "benchmark must pass --scratch-path to discovery and execution"
    [[ "$(grep -Fc "argument: $fixture_root/build output" "$log_file")" == "2" ]] \
        || fail "benchmark must preserve the path-with-spaces argument"
}

test_benchmark_requires_expected_test_count() {
    local fixture_root="$TEMP_DIR/benchmark count fixture"
    local bin_dir="$fixture_root/bin"
    mkdir -p "$bin_dir"
    cp "$FIXTURES_DIR/benchmark/swift" "$bin_dir/swift"
    chmod +x "$bin_dir/swift"

    expect_failure "Expected 13 TUIPerformanceTests, discovered 11." \
        env PATH="$bin_dir:$PATH" BENCHMARK_PERFORMANCE_TEST_COUNT=11 \
        BENCHMARK_SWIFT_LOG="$fixture_root/swift.log" \
        "$PROJECT_DIR/scripts/swifttui-benchmark.sh"
}
...
run_case performance-xcode test_ci_requires_performance_xcode_selection
run_case performance-version test_ci_requires_performance_swift_version_assertion
run_case module-boundaries test_module_test_boundaries_are_enforced
run_case benchmark-arguments test_benchmark_preserves_optional_arguments
run_case benchmark-count test_benchmark_requires_expected_test_count
run_case versioned-consumer "$PROJECT_DIR/scripts/tests/test-versioned-consumer.sh"
