...
./scripts/tests/test-api-snapshot-coverage.sh
./scripts/validate-api-snapshot-coverage.sh
...
./scripts/swifttui-fuzz.sh "${SWIFT_ARGUMENTS[@]}"
if [[ -z "$API_TOOL" ]]; then
...
if [[ -n "${SWIFTTUI_TAGGED_VERSION:-}" ]]; then
    ./scripts/verify-versioned-consumer.sh --tagged "$SWIFTTUI_TAGGED_VERSION"
else
...
TEST_COUNT="$(
    ./scripts/update-test-count.sh \
        ...
        --expected-test-target TUIFoundationTests \
        --expected-test-target TUITerminalTests \
        --expected-test-target TUIRendererTests \
        --expected-test-target TUIViewGraphTests \
        --expected-test-target TUILayoutTests \
        --expected-test-target TUIAnimationTests \
        --expected-test-target TUIControlsTests \
        --expected-test-target TUIDesignTests \
        --expected-test-target TUIRichTextTests \
        --expected-test-target TUIAgentUITests \
        --expected-test-target TUIRuntimeTests \
        --expected-test-target SwiftTUIIntegrationTests \
        --expected-test-target SwiftTUIShowcaseSmokeTests \
        --expected-test-target TUIPerformanceTests
)"
