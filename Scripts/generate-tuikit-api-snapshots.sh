...
PUBLIC_API_PRODUCTS="$PROJECT_DIR/Tools/APICompatibility/Configuration/public-api-products.tsv"
PLATFORM_EXCEPTIONS="$PROJECT_DIR/Tools/APICompatibility/Configuration/api-platform-exceptions.tsv"
...
cd "$PROJECT_DIR"
"$PROJECT_DIR/scripts/validate-api-snapshot-coverage.sh" "$PROJECT_DIR"
...
MODULES=(TUIkit TUIkitCore TUIkitImage TUIkitStyling TUIkitView)
while IFS=$'\t' read -r product module; do
    if awk -F '\t' -v module="$module" -v platform="$PLATFORM" \
        '$1 == module && $2 == platform { found = 1 } END { exit !found }' "$PLATFORM_EXCEPTIONS"; then
        printf 'Skipping API snapshot for %s on %s: ' "$module" "$PLATFORM"
        awk -F '\t' -v module="$module" -v platform="$PLATFORM" \
            '$1 == module && $2 == platform { print $3 }' "$PLATFORM_EXCEPTIONS"
        continue
    fi
    MODULES+=("$module")
done < "$PUBLIC_API_PRODUCTS"
