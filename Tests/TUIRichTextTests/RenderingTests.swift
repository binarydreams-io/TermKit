...
struct RichTextRenderingTests {
    ...
    func richTextLeaf() throws {
        ...
        var surface = TUIRenderer.Surface(size: CellSize(width: 4, height: 1))
        var resources = ControlRenderResources()

        let semantics = try richText.paint(
            into: &surface,
            context: PaintContext(origin: CellPoint(x: 1), clip: surface.bounds),
            resources: &resources
        )
    ...
    func diffCellsAtRepresentativeWidths() throws {
        ...
        #expect(narrowCells.first { $0.role == .diffRemoved }?.style.background != narrowCells.first { $0.role == .diffAdded }?.style.background)
