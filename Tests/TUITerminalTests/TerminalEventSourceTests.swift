...
struct TerminalEventSourceTests {
    #if canImport(Darwin) || canImport(Glibc)
    @Test("PTY input wakes the event source")
    func reportsPTYInputReadiness() throws {
        let pty = try PTYPair()
    ...
    @Test("PTY input delivers bracketed paste and SGR mouse events")
    func parsesTerminalProtocolsFromPTYInput() throws {
        let pty = try PTYPair()
    ...
    @Test("PTY resize signal delivers the updated terminal size")
    func deliversPTYResize() throws {
        let pty = try PTYPair()
        let originalAttributes = try pty.attributesSnapshot()
    ...
    @Test("Input readiness uses the supplied descriptor")
    func reportsInputReadiness() throws {
    ...
    @Test("Unread wake requests are coalesced")
    func coalescesWakeRequests() throws {
        let descriptors = try makePipe()
        defer { closePipe(descriptors) }
        let source = try TerminalEventSource(inputFileDescriptor: descriptors[0])
    ...
    @Test("A nil timeout blocks until an explicit wake", .timeLimit(.minutes(1)))
    func nilTimeoutBlocksUntilWake() throws {
        let descriptors = try makePipe()
        ...
        let waitQueue = DispatchQueue(label: "TUITerminalTests.blocking-wait")
        finished.enter()
        waitQueue.async {
            started.signal()
    ...
    @Test("Only one source can own signal handlers")
    func rejectsSecondSignalOwner() throws {
    ...
    @Test("Signals map to events and previous handlers are restored")
    func mapsSignalAndRestoresHandlers() throws {
        let descriptors = try makePipe()
        defer { closePipe(descriptors) }
    ...
    #if canImport(Darwin) || canImport(Glibc)
    @Test("Termination signal restores PTY state through the event source")
    func terminationSignalRestoresPTY() throws {
        let pty = try PTYPair()
        let originalAttributes = try pty.attributesSnapshot()
        let session = TerminalSession(transport: makeTerminalTestTransport(pty: pty))
        let start = try pty.captureOutput(exactByteCount: terminalTestActivationBytes.count) {
    ...
    @Test("Suspend and resume signals restore and reactivate a PTY session")
    func signalDrivenSuspendResume() throws {
        let pty = try PTYPair()
        let originalAttributes = try pty.attributesSnapshot()
        let session = TerminalSession(transport: makeTerminalTestTransport(pty: pty))
        let start = try pty.captureOutput(exactByteCount: terminalTestActivationBytes.count) {
