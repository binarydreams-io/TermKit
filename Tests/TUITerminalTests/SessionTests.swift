...
struct SessionTests {
    @Test("Session owns raw mode and protocol cleanup")
    func sessionOwnsTerminalModes() throws {
    ...
    @Test("Suspend and resume restore modes and request repaint")
    ...
    @Test("Presentation uses synchronized output only after probe proof")
    func presentationUsesProbedCapability() throws {
        let script = TerminalPOSIXScript()
        let session = makeSession(script: script)
        try session.start()
    ...
    @Test("Permanent presentation failure restores terminal state")
    func presentationFailureRestoresSession() throws {
    ...
    @Test("Scoped session restores after thrown work")
    func scopedSessionRestoresAfterFailure() {
    ...
    @Test("Non-terminal input fails before termios changes")
    func nonTerminalInputFails() {
        let script = TerminalPOSIXScript()
    ...
    @Test("Signal events drive suspend and resume outside handlers")
    func signalEventsDriveTransitions() throws {
        let script = TerminalPOSIXScript()
        let session = makeSession(script: script)
        try session.start()
