...
struct TerminalAttributesSnapshot: Equatable {
  ...
  init(_ attributes: inout termios) {
    self.inputFlags = attributes.c_iflag
    self.outputFlags = attributes.c_oflag
    self.controlFlags = attributes.c_cflag
    self.localFlags = attributes.c_lflag
    self.inputSpeed = cfgetispeed(&attributes)
    self.outputSpeed = cfgetospeed(&attributes)
    self.controlCharacters = withUnsafePointer(to: &attributes.c_cc) { pointer in
...
final class PTYPair: @unchecked Sendable {
  ...
  init() throws {
    ...
    do {
      ...
      self.slaveFileDescriptor = slave
    } catch {
      _ = close(master)
      throw error
    }
  ...
  func writeToMaster(_ bytes: [UInt8], timeoutMilliseconds: Int32 = 1000) throws {
  ...
  func readFromMaster(exactByteCount: Int, timeoutMilliseconds: Int32 = 1000) throws -> [UInt8] {
    var received: [UInt8] = []
  ...
  private func wait(
    ...
    while true {
      ...
      if result > 0 {
        if Int32(descriptor.revents) & Int32(events) != 0 {
          return
        }
