...
struct TerminalSizeReaderTests {
  ...
  @Test
  func `PTY window size is read through ioctl`() throws {
    let pty = try PTYPair()
  ...
  @Test
  func `Injected ioctl returns a terminal size`() throws {
  ...
  @Test
  func `Non-positive dimensions are rejected`() {
    let reader = TerminalSizeReader(
  ...
  @Test
  func `Ioctl failures preserve errno`() {
    let reader = TerminalSizeReader(
      fileDescriptor: STDOUT_FILENO,
      systemCalls: TerminalSizeSystemCalls(
  ...
  @Test
  func `Ioctl retries after interruption`() throws {
