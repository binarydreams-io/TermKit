...
struct ImageRenderingTests {
  @Test
  func `Two vertical pixels render as one colored upper block`() throws {
  ...
  @Test
  func `Odd image height composites its missing lower pixel against the background`() throws {
  ...
  @Test
  func `Transparent pixels composite against the explicit background`() throws {
    let raster = try RasterImage(
      width: 1,
      height: 2,
      pixels: [
  ...
  @Test
  func `Monochrome rendering uses block glyphs and default styles`() throws {
    let raster = try RasterImage(
      width: 2,
      height: 2,
      pixels: [
  ...
  @Test
  func `Fit and fill use centered deterministic sampling`() throws {
    let raster = try RasterImage(
      width: 4,
      height: 2,
      pixels: [
  ...
  func `Image colors use the terminal encoder mode`(
