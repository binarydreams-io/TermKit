...
struct RasterImageTests {
  @Test
  func `Raster images validate dimensions and pixel count`() throws {
  ...
  @Test
  func `PNG alpha decodes to straight RGBA8`() throws {
  ...
  @Test
  func `JPEG decodes to opaque RGBA8`() throws {
  ...
  @Test
  func `Unsupported and truncated inputs are rejected`() throws {
  ...
  @Test
  func `PNG preflight rejects animation and unsafe dimensions`() throws {
  ...
  @Test
  func `Data and local file reads enforce encoded byte limits`() throws {
    ...
    #expect(throws: RasterImageError.unreadableFile) {
      _ = try RasterImage(contentsOf: #require(URL(string: "https://example.com/image.png")))
    }
