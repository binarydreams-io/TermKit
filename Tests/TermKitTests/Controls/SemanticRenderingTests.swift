...
struct SemanticRenderingTests {
  @Test
  @MainActor
  func `Progress bar clips deterministically and exposes its value`() throws {
  ...
  @Test
  @MainActor
  func `Adjustable progress bar clamps semantic changes`() {
  ...
  @Test
  @MainActor
  func `Text leaf retains its primitive and measures terminal cells`() throws {
    let text = Text("界x")
  ...
  @Test
  @MainActor
  func `Button leaf publishes interaction metadata`() throws {
  ...
  @Test
  func `Text paints cells and publishes a semantic node`() throws {
  ...
  @Test
  func `Unicode clusters use foundation widths and preserve adjacent surface cells`() throws {
  ...
  @Test
  func `A clipped wide grapheme paints a visible one-cell fallback`() throws {
