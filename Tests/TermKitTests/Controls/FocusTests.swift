...
struct FocusTests {
  @Test
  func `A trapping scope cycles only through its enabled targets`() {
  ...
  @Test
  func `Closing a scope restores the previous enabled target`() {
    let manager = FocusManager()
  ...
  @Test
  func `Nested modal overlays dismiss in stack order and restore focus`() {
    let manager = FocusManager()
    manager.register(FocusTarget(id: "source"))
  ...
  @Test
  func `Modal focus order follows visual z-order and Escape restores focus`() {
    let manager = FocusManager()
    manager.register(FocusTarget(id: "source"))
