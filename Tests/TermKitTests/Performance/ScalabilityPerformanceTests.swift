...
struct ScalabilityPerformanceTests {
  @Test
  func `A 10,000-item conversation renders only its lazy visible range`() throws {
    let itemCount = 10000
  ...
  @Test
  func `One tool-row mutation stays local in a mounted 10,000-item transcript`() throws {
    let context = try AgentRenderContext(width: 120, scheme: .dark)
    let changedIndex = 5000
    ...
    print(result.report(budgetMilliseconds: 8))
    #expect(harness.transcriptItemCount == 10000)
  ...
  @Test
  func `One runtime row invalidation scans one row instead of the full surface`() throws {
  ...
  @Test
  func `Interner growth is bounded and rebuild retains only live cells`() throws {
...
private final class ToolTranscriptHarness {
  ...
  init(itemCount: Int, visibleIndex: Int, context: AgentRenderContext) throws {
    ...
    self.viewport = ConversationViewport(
  ...
  var mountedItemCount: Int {
    graph.root?.children.count ?? 0
  }
