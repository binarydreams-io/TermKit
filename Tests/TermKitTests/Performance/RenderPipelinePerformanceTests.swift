...
struct RenderPipelinePerformanceTests {
  @Test
  func `Localized 120x40 animation stays below the p95 frame budget`() throws {
  ...
  @Test
  func `Declarative paint animation skips reconciliation and layout`() throws {
  ...
  @Test
  func `A 200x60 full repaint stays below one 60 Hz frame`() throws {
  ...
  @Test
  func `An unchanged frame emits no operations and performs no write`() throws {
  ...
  @Test
  func `An ordinary changed frame performs one logical in-memory write`() throws {
    let harness = try RuntimeFrameHarness(size: CellSize(width: 120, height: 40))
