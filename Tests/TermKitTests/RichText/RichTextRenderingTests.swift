...
struct RichTextRenderingTests {
  @Test
  @MainActor
  func `Rich-text leaf measures, retains its primitive, and paints wide cells`() throws {
  ...
  @Test
  func `Styled text resolves semantic colors and packs wide cells for both schemes`() throws {
  ...
  @Test
  func `Markdown paints semantic constructs and code backgrounds`() throws {
    let source = """
    # Title
    ...
    let cells = rendered.cells.rows.flatMap(\.self)
  ...
  @Test
  func `Malformed Markdown uses source text and attaches a diagnostic`() throws {
    let source = "```swift\nlet value = 1"
    ...
    #expect(rendered.cells.rows.flatMap(\.self).filter { $0.grapheme != " " }.allSatisfy { $0.role == .body })
    #expect(rendered.semantics.children.first?.role == .status)
  }

  @Test
  func `Malformed Markdown preserves semantic prefix blocks`() throws {
  ...
  @Test
  func `Code block paints title, gutter, syntax, selection, and semantic state`() throws {
    ...
    #expect(rendered.cells.rows.flatMap(\.self).contains { $0.role == .keyword })
    #expect(rendered.cells.rows.flatMap(\.self).contains { $0.isSelected })
  ...
  @Test
  func `Unified and side-by-side diffs paint gutters and semantic backgrounds`() throws {
    let source = """
    --- a/file.txt
    +++ b/file.txt
    @@ -1,2 +1,2 @@
    ...
    let wideCells = wide.cells.rows.flatMap(\.self)
