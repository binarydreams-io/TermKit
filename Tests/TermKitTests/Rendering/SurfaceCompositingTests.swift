...
struct SurfaceCompositingTests {
  @Test func `bounded surface uses absolute coordinates`() throws {
  ...
  @Test func `nested transparent compositing preserves alpha until opaque destination`() throws {
  ...
  @Test func `half opacity blends white over black in linear SRGB`() throws {
    var graphemes = GraphemeInterner()
    var styles = StyleInterner()
  ...
  @Test func `zero opacity preserves destination and one uses source`() throws {
    var graphemes = GraphemeInterner()
    var styles = StyleInterner()
  ...
  @Test func `clip uses actual lower layer styles`() throws {
    var graphemes = GraphemeInterner()
    var styles = StyleInterner()
  ...
  @Test func `wide source and destination atoms do not cross clip boundaries`() throws {
    var graphemes = GraphemeInterner()
    var styles = StyleInterner()
...
private func explicitDefaults(_ color: Color?, role: CellColorRole) -> RGBA {
  ...
  guard case let .rgba(rgba) = color else {
...
private func rgba(_ color: Color?) -> RGBA? {
  guard case let .rgba(rgba)? = color else { return nil }
  return rgba
}
