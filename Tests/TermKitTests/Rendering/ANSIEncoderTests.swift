...
struct ANSIEncoderTests {
  @Test
  func `Monochrome mode emits attributes without color SGR codes`() throws {
  ...
  @Test func `true color encoding frames synchronized output`() throws {
    var graphemes = GraphemeInterner()
  ...
  @Test func `encoder omits redundant cursor and style sequences`() throws {
    var graphemes = GraphemeInterner()
  ...
  func `fallback color modes`(mode: ANSIColorMode, expected: String) throws {
    var graphemes = GraphemeInterner()
  ...
  @Test func `failed encoding does not commit encoder state`() throws {
  ...
  @Test func `empty diff does not emit sync frame`() throws {
    var encoder = ANSIEncoder()
