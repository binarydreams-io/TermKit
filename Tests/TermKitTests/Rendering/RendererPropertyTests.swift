...
struct RendererPropertyTests {
  @Test func `seeded unicode surfaces preserve wide cell invariants`() {
    runRendererPropertyCases(count: 256, baseSeed: 0xA11CE5E15EED0001) { random in
  ...
  @Test func `arbitrary write clear and compose sequences preserve wide cell invariants`() {
    runRendererPropertyCases(count: 256, baseSeed: 0xA11CE5E15EED0002) { random in
      var interner = GraphemeInterner()
      let graphemes = try rendererGraphemes(interner: &interner)
  ...
  @Test func `random damage diffs replay to next semantic surface`() {
    runRendererPropertyCases(count: 320, baseSeed: 0xA11CE5E15EED0003) { random in
      var interner = GraphemeInterner()
      let graphemes = try rendererGraphemes(interner: &interner)
  ...
  @Test func `random resize and overlapping layer sequences preserve wide cell invariants`() {
    runRendererPropertyCases(count: 192, baseSeed: 0xA11CE5E15EED0004) { random in
      var interner = GraphemeInterner()
      let graphemes = try rendererGraphemes(interner: &interner)
...
private func runRendererPropertyCases(
  ...
  let seeds =
    configuredSeed.map { [$0] }
      ?? (0 ..< count).map {
        baseSeed &+ UInt64($0) &* 0x9E3779B97F4A7C15
      }
