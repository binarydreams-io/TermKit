...
struct RuntimeTests {
  @Test
  func `Resize updates terminal-size environment dependencies`() throws {
  ...
  @Test
  func `Application commands precede focused control dispatch`() throws {
  ...
  @Test
  func `Progress semantics adjust without motion or idle frame demand`() throws {
  ...
  @Test
  func `Bound agent prompt renders and handles focused runtime input`() throws {
  ...
  @Test
  func `Agent prompt metadata insertion transitions only metadata paint for 150 milliseconds`() throws {
  ...
  @Test
  func `Prompt runtime handles character selection, deletion, multiline replacement, and rejected paste`() throws {
  ...
  @Test
  func `Running reasoning requests shared timeline cadence and static states stop`() throws {
    let running = Runtime(
  ...
  @Test
  func `Viewport semantic actions and autocomplete input execute through runtime`() throws {
  ...
  @Test
  func `Agent components execute keyboard, pointer, focus, and dismissal actions through runtime`() throws {
  ...
  @Test
  func `Agent component semantics expose actions only when a runtime adapter can execute them`() throws {
  ...
  @Test
  func `Permission prompt traps runtime input and executes the focused choice`() throws {
  ...
  @Test
  func `Question prompt edits current steps and navigates through runtime input`() throws {
  ...
  @Test
  func `Declarative runtime lays out, paints, focuses, activates, and preserves identity`() throws {
  ...
  @Test
  func `Primitive paint receives inherited and locally overridden environment values`() throws {
    let session = FakeTerminalSession()
    let presenter = FramePresenter(session: session)
    let runtime = Runtime(
  ...
  @Test
  func `Declarative view observes an animation transaction`() throws {
  ...
  @Test
  func `State mutation carries its animation transaction to the runtime`() throws {
    let presenter = FramePresenter(session: FakeTerminalSession())
    let runtime = Runtime(
  ...
  @Test
  func `Idle runtime has no next deadline`() throws {
  ...
  @Test
  func `Concurrent invalidation channel coalesces regions and enters the UI executor`() async throws {
    let runtime = Runtime(
  ...
  @Test
  func `Periodic timeline requests its next absolute deadline and changes content`() throws {
  ...
  @Test
  func `Explicit timeline becomes idle after its last entry`() throws {
  ...
  @Test
  func `Reduced motion timeline renders a static fallback and stays idle`() throws {
  ...
  @Test
  func `An unchanged invalidated runtime frame does not write`() throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `Runtime frame stats contain measured stages and counters`() throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `Missed frame budgets accumulate`() throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `Invalidations coalesce without stale frames`() throws {
    let view = TestRuntimeView(text: "a")
    let session = FakeTerminalSession()
    let runtime = makeRuntime(view: view, session: session)
    try runtime.start()
  ...
  @Test
  func `Resize forces a full repaint`() throws {
    let view = TestRuntimeView(text: "a")
    let session = FakeTerminalSession()
    let runtime = makeRuntime(view: view, session: session)
    try runtime.start()
  ...
  @Test
  func `Resume forces a full repaint`() throws {
    let runtime = makeRuntime(view: TestRuntimeView(text: "a"), session: FakeTerminalSession())
    try runtime.start()
    _ = try runtime.renderIfDue(at: .zero)
    try runtime.suspend()
  ...
  @Test
  func `Reduced motion samples the final state and stays idle`() throws {
  ...
  @Test
  func `Input and signal callbacks run on the runtime state machine`() throws {
  ...
  @Test
  func `Run waits off the main actor and dispatches parsed input`() async throws {
    var inputs: [TerminalInputEvent] = []
    let session = FakeTerminalSession()
  ...
  func `Cancellation stops the runtime and restores the terminal session`() async throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `Runtime startup applies a synchronized-output response`() async throws {
  ...
  @Test
  func `A synchronized-output timeout selects fallback and records a diagnostic`() async throws {
    let session = FakeTerminalSession(synchronizedOutput: .unknown)
  ...
  @Test
  func `A mandatory synchronized-output timeout throws a typed error`() async throws {
    let session = FakeTerminalSession(synchronizedOutput: .unknown)
  ...
  func `Unsupported and oversized synchronized-output responses select fallback`(
    bytes: [UInt8],
  ...
  @Test(.timeLimit(.minutes(1)))
  func `Idle invalidation wakes the runtime without a timer`() async throws {
    let session = FakeTerminalSession()
    let view = TestRuntimeView(text: "a")
    let eventSource = FakeRuntimeEventSource()
  ...
  @Test
  func `Input closure stops and restores the runtime once`() async throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `A frame failure stops and cleans up the terminal session`() throws {
    let session = FakeTerminalSession()
    let runtime = Runtime(
  ...
  @Test
  func `A layout failure rolls back the staged graph and lifecycle`() throws {
    let view = TestRuntimeView(text: "a")
    let session = FakeTerminalSession()
    let runtime = makeRuntime(view: view, session: session)
    try runtime.start()
  ...
  @Test
  func `A paint failure rolls back the staged graph and lifecycle`() throws {
    let view = TestRuntimeView(text: "a")
    let session = FakeTerminalSession()
    let runtime = makeRuntime(view: view, session: session)
    try runtime.start()
  ...
  @Test
  func `A presentation failure rolls back the staged graph and lifecycle`() throws {
    let view = TestRuntimeView(text: "a")
    let session = FakeTerminalSession()
    let runtime = makeRuntime(view: view, session: session)
    try runtime.start()
  ...
  private func foregroundRGBA(at point: CellPoint, presenter: FramePresenter) -> RGBA? {
    guard let surface = presenter.frontSurface,
          ...
          case let .rgba(color)? = style.foreground
    else { return nil }
    return color
  }
...
private final class RuntimePromptTransitionModel {
  ...
  init(animationsEnabled: Bool = true) {
    self.configuration = AgentPromptConfiguration(animationsEnabled: animationsEnabled)
  }
}
