...
struct FramePresenterTests {
  @Test
  func `An unchanged frame does not write`() throws {
  ...
  @Test
  func `A frame is one logical session write`() throws {
    let session = FakeTerminalSession()
  ...
  @Test
  func `Damage limits diff scanning and output to one cell`() throws {
    let session = FakeTerminalSession()
    let presenter = FramePresenter(session: session)
    _ = try session.start()
    _ = try presenter.present(makeSurface("abc", resources: presenter))
  ...
  @Test
  func `Damage counts requested cells even when they are unchanged`() throws {
    let session = FakeTerminalSession()
    let presenter = FramePresenter(session: session)
    _ = try session.start()
    let surface = try makeSurface("abc", resources: presenter)
  ...
  @Test
  func `Damage remains distinct from forced repaint scanner work`() throws {
    let session = FakeTerminalSession()
    let presenter = FramePresenter(session: session)
    _ = try session.start()
  ...
  @Test
  func `Synchronized framing is delegated to the terminal session`() throws {
  ...
  @Test
  func `Real terminal session presents one synchronized physical write`() throws {
    ...
    #expect(
      writes[0].suffix(TerminalTransport.endSynchronizedOutput.count).elementsEqual(
  ...
  @Test
  func `Interner rebuild retains live cells and forces repaint`() throws {
    let limits = InternerLimits(
      rebuildEntryCount: 4,
      maximumEntryCount: 8,
      rebuildByteCount: 1024,
      maximumByteCount: 2048
    )
    let session = FakeTerminalSession()
  ...
  private func makeSurface(_ text: String, resources presenter: FramePresenter) throws -> Surface {
    var surface = Surface(size: CellSize(width: text.count, height: 1))
...
private final class RuntimeTerminalPOSIXScript: @unchecked Sendable {
  private let queue = DispatchQueue(label: "RuntimeTests.terminal-posix")
...
final class FakeTerminalSession: RuntimeTerminalSession {
  ...
  init(synchronizedOutput: TerminalCapabilitySupport = .unsupported) {
    self.capabilities = TerminalCapabilities(color: .trueColor, synchronizedOutput: synchronizedOutput)
  }
