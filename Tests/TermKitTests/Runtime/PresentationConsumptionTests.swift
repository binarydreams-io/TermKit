...
struct PresentationConsumptionTests {
    ...
    func colorAndOpacity() throws {
        ...
        guard case .rgba(let rgba) = foreground else {
    ...
    func opacityComposite() throws {
        ...
        let style = try style(at: .zero, presenter: presenter)
        guard case .rgba(let rgba)? = style.foreground else {
    ...
    func malformedPresentationTarget() throws {
        ...
        } catch RuntimeError.invalidPresentationValue(let property, let value) {
    ...
    func focusHighlight() throws {
        ...
        guard case .rgba(let color)? = style.foreground else {
...
extension TimeInstant {
    fileprivate static func milliseconds(_ value: Int64) -> TimeInstant { .zero.advanced(by: .milliseconds(value)) }
    fileprivate static func seconds(_ value: Int64) -> TimeInstant { .zero.advanced(by: .seconds(Double(value))) }
}
