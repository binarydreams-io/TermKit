...
struct AgentRenderingTests {
  @Test
  @MainActor
  func `Agent component leaf paints production output and offsets semantics`() throws {
    let view = try AgentComponentView(
      ToastPresentation(kind: .info, message: "界 ready"),
      scheme: .dark
    )
    let descriptor = try #require(view.graphBody.first)
    var surface = Surface(size: CellSize(width: 14, height: 3))
  ...
  @Test
  @MainActor
  func `Bound prompt leaf measures and paints production output`() throws {
    ...
    var surface = Surface(size: CellSize(width: 40, height: 10))
    var resources = ControlRenderResources()

    let semantics = try renderable.paint(
      into: &surface,
      context: PaintContext(clip: CellRect(x: 0, y: 0, width: 40, height: 10), origin: .zero),
      resources: &resources
    )
  ...
  @Test
  func `Toast renders a semantic accent rail and status`() throws {
    let output = try ToastPresentation(kind: .error, message: "Build failed").render(
      in: AgentRenderContext(width: 40, scheme: .dark)
    )
  ...
  @Test
  @MainActor
  func `Prompt metadata is a separate semantic paint leaf`() throws {
    let model = RenderingPromptModel()
    let prompt = AgentPrompt<String>(
      model.binding,
    ...
    #expect(try editor.prompt.render(in: AgentRenderContext(width: 40, scheme: .dark)).plainText.contains("swift-6") == false)
  }

  @Test
  func `User message applies its configured semantic rail color`() throws {
    let output = try UserMessageCard<String>(text: "Hello", agentColor: .success).render(
      in: AgentRenderContext(width: 40, scheme: .dark)
    )

    #expect(output.cells.rows.flatMap(\.self).compactMap(\.self).first { $0.grapheme == "▌" }?.foregroundRole == .success)
  }

  @Test
  func `Completed reasoning renders its optional summary`() throws {
    let output = try ReasoningDisclosure(
      phase: .completed,
      body: "Details",
  ...
  nonisolated static let widths = [40, 80, 120, 180]
  nonisolated static let schemes: [ColorScheme] = [.light, .dark]

  @Test(arguments: widths, schemes)
  @MainActor
  func `Core components produce semantic cell grids`(width: Int, scheme: ColorScheme) throws {
    ...
    let components: [any AgentComponentRenderable] = [
      ...
      UserMessageCard(
        ...
        attachments: ["Package.swift"],
        timestamp: "10:42",
        isQueued: true
      ),
      ReasoningDisclosure(phase: .running, body: "Checking cell widths", summary: "Inspecting renderer APIs"),
      ToolCallRow(
        id: 1,
        label: "Run swift test",
        state: .failed,
        errorBody: "Compiler exited with status 1",
        isErrorExpanded: true
      ),
      ToolResultPanel(title: "Read", content: "48 lines", presentation: .panel),
      ShellResult(
        command: "swift test",
        workingDirectory: "~/TermKit",
        output: "one\ntwo\nthree",
        isRunning: true
      ),
  ...
  @Test(arguments: widths, schemes)
  func `Conversation, assistant, questions, and sidebar perform executable layout`(width: Int, scheme: ColorScheme) throws {
    let context = try AgentRenderContext(width: width, scheme: scheme)
    let viewportState = ConversationViewportState(viewportExtent: 2, itemExtent: 1, itemCount: 3)
    let question = Question(
      id: "target",
      title: "Select a target",
      kind: .singleSelection,
      options: [QuestionOption(id: "agent", label: "Agent UI")],
    ...
    let components: [any AgentComponentRenderable] = [
      ...
      AssistantMessage(
        ...
        reasoning: ["Checked diagnostics"],
        toolActivity: ["Read Package.swift"],
  ...
  @Test
  @MainActor
  func `Focused, disabled, busy, error, and reduced-motion states are semantic`() throws {
    ...
    let selected = PromptAutocomplete(
  ...
  @Test(arguments: widths, schemes)
  func `Background pulse becomes static under reduced motion`(width: Int, scheme: ColorScheme) throws {
  ...
  func `Prompt cell-grid snapshot adapts at canonical widths and themes`(width: Int, scheme: ColorScheme) throws {
    ...
    #expect(output.theme.scheme == scheme)
    #expect(output.cells.rows.flatMap(\.self).compactMap(\.self).contains { $0.role == .listMarker })
  }
