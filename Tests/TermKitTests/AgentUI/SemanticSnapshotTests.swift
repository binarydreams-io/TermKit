...
struct SemanticSnapshotTests {
  ...
  @MainActor
  func `Agent components match semantic snapshots`(width: Int, scheme: ColorScheme) throws {
  ...
  @Test(arguments: widths, schemes)
  @MainActor
  func `Interactive states match semantic snapshots`(width: Int, scheme: ColorScheme) throws {
    let source = try Self.matrixSnapshot(
      width: width,
      scheme: scheme,
      entries: Self.interactiveStates(width: width, scheme: scheme)
    )
  ...
  @Test
  func `Semantic snapshot preserves sparse cell and tree data`() throws {
    let theme = try SemanticTheme.standard.resolve(scheme: .dark)
    let grid = SemanticCellGrid(
      width: 3,
      rows: [
        [
          SemanticCell(
    ...
    let output = AgentRenderOutput(
      ...
      semantics: SemanticNode(
        ...
        actions: [.dismiss, .activate],
        frame: CellRect(x: 0, y: 0, width: 3, height: 1),
    ...
    #expect(
      output.semanticSnapshot.contains(
  ...
  private static func record(_ snapshot: String, named name: String) throws {
    guard ProcessInfo.processInfo.environment["TERMKIT_RECORD_SNAPSHOTS"] == "1" else { return }
  ...
  private static func matrixSnapshot(
    ...
    if let first = entries.first {
      lines.append(
        contentsOf: first.1.semanticSnapshot.split(separator: "\n").lazy
    ...
    for (name, output) in entries {
      ...
      lines.append(
        contentsOf: compact(output.semanticSnapshot).split(separator: "\n").lazy
          .filter {
  ...
  private static func canonicalComponents() -> [(String, any AgentComponentRenderable)] {
    ...
    return [
      ...
      ("toast", ToastPresentation(kind: .error, message: "Build failed")),
      (
        "user-message",
        UserMessageCard(
          text: "Inspect the semantic renderer.",
      ...
      ("shell-result", ShellResult(command: "swift test", workingDirectory: "~/TermKit", output: "one\ntwo")),
      (
        "diff",
        AgentDiffContentView(
          unifiedDiff: """
          --- a/Renderer.swift
          +++ b/Renderer.swift
          @@ -1 +1 @@
      ...
      (
        ...
        ConversationViewport(
          ...
          state: ConversationViewportState(viewportExtent: 2, itemExtent: 1, itemCount: 3)
        )
      ),
      (
        "assistant-message",
      ...
      (
        "pulse",
        BackgroundPulse<BackgroundPulseFrame>(
  ...
  private static func interactiveStates(
    ...
    return try [
      ...
      (
        "hovered",
        UserMessageCard<String>(text: "Inspect the renderer.", isHovered: true)
          .render(in: context())
      ),
      ("selected", autocomplete().render(in: context())),
      ("disabled-prompt", prompt(isEnabled: false).render(in: context())),
      (
        "disabled-menu",
        PromptAutocomplete(state: autocomplete().state, isEnabled: false)
          .render(in: context())
      ),
      ("busy-prompt", prompt(isBusy: true).render(in: context())),
      ("busy-shell", ShellResult(command: "swift test", isRunning: true).render(in: context())),
      (
        "error",
        ToolCallRow(
          id: 1,
      ...
      (
        "reduced-motion-reasoning",
        ReasoningDisclosure(phase: .running, body: "Cells", summary: "Renderer")
          .render(in: context(reduceMotion: true))
      ),
      (
      ...
      ("focused-dialog", permission().render(in: context()))
    ]
  }
