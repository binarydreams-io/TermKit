...
struct DeclarativeAnimationTests {
  ...
  @Test
  func `Value-scoped animation creates and retargets a mounted-node track`() throws {
    let graph = ViewGraph()
  ...
  @Test
  func `Removing an animatable property removes its mounted-node track`() throws {
    let graph = ViewGraph()
    try commit(ValueView(value: 0, property: property), to: graph, at: .zero)
    try commit(ValueView(value: 10, property: property), to: graph, at: .zero)
    let leaf = try #require(graph.root?.children.first?.children.first)
  ...
  @Test
  func `Removal transition stays noninteractive until its track completes`() throws {
    let graph = ViewGraph()
  ...
  @Test
  func `Insertion transition samples its midpoint and becomes active at completion`() throws {
    let graph = ViewGraph()
    let transaction = Transaction(
      animation: .linear(duration: .seconds(1)),
      animationTime: .zero
  ...
  @Test
  func `Reinsertion reverses removal continuously and ignores stale completion`() throws {
    let graph = ViewGraph()
    ...
    let midpoint = TimeInstant.zero.advanced(by: .milliseconds(1500))
  ...
  @Test
  func `Disabled transition animation completes insertion and removal immediately`() throws {
    let graph = ViewGraph()
    let transaction = Transaction(
      animation: .linear(duration: .seconds(1)),
