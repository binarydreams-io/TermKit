...
struct CellAndInternerTests {
  @Test func `packed cell round trips all fields`() {
    let cell = PackedCell(
      graphemeID: GraphemeID(rawValue: 0x123456),
      styleID: StyleID(rawValue: 0x654321),
      displayWidth: 2,
      flags: [.transparent, .explicitBlank]
    )

    #expect(MemoryLayout<PackedCell>.size == 8)
    #expect(cell.graphemeID == GraphemeID(rawValue: 0x123456))
    #expect(cell.styleID == StyleID(rawValue: 0x654321))
    #expect(cell.displayWidth == 2)
    #expect(cell.flags == [.transparent, .explicitBlank])
  }

  @Test func `grapheme interner accepts extended clusters and keeps stable I ds`() throws {
  ...
  @Test func `grapheme interner enforces hard limit`() throws {
  ...
  @Test func `rebuild returns live identifier map`() throws {
    var interner = GraphemeInterner()
  ...
  @Test func `style interner deduplicates styles`() throws {
  ...
  @Test func `bounded buffer drops oldest elements`() {
