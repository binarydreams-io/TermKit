...
struct PresentationPropertyTests {
    ...
    @Test("The property catalog exposes stable keys, types, and invalidation classes")
    func catalog() {
    ...
    @Test("Presentation vectors permit intermediate arithmetic values")
    func vectors() {
        #expect(CellVector.interpolated(from: .zero, to: CellVector(x: 3, y: -1), progress: 0.5) == CellVector(x: 1.5, y: -0.5))
        #expect(CellDimensions.interpolated(from: .zero, to: CellDimensions(width: 3, height: -1), progress: 0.5) == CellDimensions(width: 1.5, height: -0.5))
        #expect(FloatingEdgeInsets.interpolated(
            from: .zero,
    ...
    @Test("Presentation target validation rejects every non-finite component")
    func finiteTargetValidation() {
    ...
    @Test("Linear color interpolation produces an sRGB midpoint near 188")
    func linearColorMidpoint() {
    ...
    @Test("A static typed property retains its target without an active animation")
    func staticTargetLookup() throws {
        let graph = ViewGraph()
    ...
    @Test("Typed tracks interpolate and retarget from their sampled presentation")
    func midpointAndRetarget() throws {
        let graph = ViewGraph()
    ...
    @Test("Sampling propagates property flags without calling the invalidation handler")
    func samplingDirtyPropagation() throws {
    ...
    @Test("A completing sample invalidates its final frame without scheduling another frame")
    func finalSample() throws {
        let graph = ViewGraph()
        try commit(value: 0, property: .opacity, to: graph, transaction: Transaction(animationTime: .zero))
        try commit(
            value: 1,
    ...
    @Test("Rollback restores an ordinary property's sampled value and running status")
    func sampledPropertyRollback() throws {
        let graph = ViewGraph()
        try commit(value: 0, property: .opacity, to: graph, transaction: Transaction(animationTime: .zero))
        try commit(
            value: 1,
        ...
        let node = try #require(graph.root)
        let frame = try graph.beginCommit(graph.prepare(
    ...
    @Test("A completed removal rolls back before completion and succeeds once on retry")
    func removalCompletionRollback() throws {
        let graph = ViewGraph()
        ...
        for action in insertion.completionActions { action() }
        ...
        for action in commitActions + retrySample.completionActions { action() }

        #expect(graph.presentationNode(withID: node.id) == nil)
        #expect(completionCount == 1)
    }

    @Test("Disabled animation is immediate and reduced motion preserves color cadence")
    func motionPolicies() throws {
    ...
    @Test("The string-key animatable value API remains compatible")
    func legacyAPI() throws {
    ...
    @Test("Declarative modifiers create typed tracks with their invalidation classes")
    func declarativeModifiers() throws {
    ...
    @Test("Coordinate convenience overloads retain floating targets")
    func coordinateOverloads() throws {
    ...
    @Test("A modifier preserves the expanded descriptor identity and evaluates its body once")
    func modifierPreservesExpansion() throws {
    ...
    @Test("Transition sampling invalidates layout only for resolved spatial effects")
    func transitionSamplingDirtyFlags() throws {
    ...
    @Test("Transition samples retain reveal and wipe edges")
    func transitionSampleEdges() {
    ...
    private func expectModifier<Value: VectorArithmetic & Hashable, Content: View>(
        _ property: PresentationProperty<Value>,
    ...
    private func commit<Content: View>(_ content: Content, to graph: ViewGraph) throws {
        try withTransaction(Transaction(animationTime: .zero)) {
            try graph.commit(graph.prepare(content))
        }
    }

    private func firstNode<Value: VectorArithmetic>(
        in node: MountedNode?,
        with property: PresentationProperty<Value>
    ) -> MountedNode? {
        guard let node else { return nil }
        if node.presentationValue(property) != nil { return node }
        for child in node.children {
            if let match = firstNode(in: child, with: property) { return match }
        }
        return nil
    }
