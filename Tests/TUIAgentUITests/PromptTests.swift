...
struct PromptTests {
    @Test(
        "Prompt layout follows the specified width policy",
        arguments: [
    ...
    func resolvedWidth(argument: (terminalWidth: Int, expectedWidth: Int)) {
    ...
    @Test("Prompt metadata transition becomes static when animation is disabled")
    @MainActor
    func metadataTransition() {
    ...
    @MainActor
    @Test("Prompt rejects large paste while preserving ordinary paste")
    func pastePolicy() {
    ...
    @MainActor
    @Test("Prompt edits character offsets and multiline selections")
    func runtimeEditing() {
    ...
    @Test("Prompt insertion replaces character offsets and places the cursor")
    func insertion() {
    ...
    @MainActor
    @Test("Autocomplete delegates filtering and navigation to SelectList")
    func autocompleteSelectListDelegation() throws {
        let suggestions = [
            PromptSuggestion(
                id: "build",
                kind: .command,
        ...
        let output = autocomplete.render(in: try AgentRenderContext(width: 40, scheme: .dark))
    ...
    @MainActor
    @Test("Autocomplete activation returns typed insertion without mutating the prompt")
    func autocompleteInsertion() throws {
        ...
        let autocomplete = PromptAutocomplete(state: PromptAutocompleteState(suggestions: [
        ...
        document.apply(try #require(returned))
        #expect(document == PromptDocument(text: "/test ", selection: PromptSelection(caret: 5)))
    }

    @MainActor
    @Test("Autocomplete retains its overlay anchor")
    func autocompleteAnchor() {
        let autocomplete = PromptAutocomplete<String>(state: PromptAutocompleteState(
            anchorColumn: 14,
            anchorRow: 6
        ))
    ...
    @MainActor
    @Test("Prompt actions emit values without owning execution")
    func actions() {
        final class Recorder {
    ...
    @MainActor
    @Test("Bound prompt reads, mutates, and submits the current document")
    func boundDocument() throws {
        ...
        #expect(renderedPrompt.render(in: try AgentRenderContext(width: 80, scheme: .dark)).plainText.contains("External"))
    ...
    @MainActor
    @Test("Prompt retains accessory state and leaf identities across updates")
    func retainedGraphIdentity() throws {
        ...
        if case .stack(let stack) = primitive {
    ...
    @MainActor
    @Test("Bound prompt supports empty accessories")
    func emptyAccessories() {
        let model = PromptModel(document: PromptDocument())
