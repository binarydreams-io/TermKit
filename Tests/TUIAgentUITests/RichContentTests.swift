...
struct AgentRichContentTests {
    @Test("Code block wrapper forwards presentation, selection, copy, wrap, and highlighting")
    @MainActor
    func codeBlockView() throws {
        ...
        let output = view.render(in: try AgentRenderContext(width: 12, scheme: .dark))
        let activated = view.activate()

        #expect(output.plainText.split(separator: "\n").first?.hasPrefix("Example") == true)
        #expect(output.cells.rows.flatMap { $0 }.compactMap { $0 }.contains { $0.role == .lineNumber })
        #expect(output.cells.rows.flatMap { $0 }.compactMap { $0 }.contains { $0.role == .type })
        #expect(output.cells.rows.flatMap { $0 }.compactMap { $0 }.contains { $0.attributes.contains(.selected) })
    ...
    @Test("Code block can disable selection and copy")
    @MainActor
    func disabledCodeBlockCapabilities() throws {
        let view = AgentCodeBlockView(
            AgentCodeBlock(text: "value", supportsSelection: false, selection: 0..<5, isCopyEnabled: false)
        )

        let output = view.render(in: try AgentRenderContext(width: 10, scheme: .light))
    ...
    @Test("Unified diff text uses width policy and semantic regions")
    @MainActor
    func unifiedDiffText() throws {
        let source = """
            --- a/file.txt
            +++ b/file.txt
            @@ -1,2 +1,2 @@
        ...
        let narrow = view.render(in: try AgentRenderContext(width: 40, scheme: .light))
        let wide = view.render(in: try AgentRenderContext(width: 121, scheme: .dark))
        let narrowCells = narrow.cells.rows.flatMap { $0 }.compactMap { $0 }
    ...
    @Test("Parsed diff honors explicit layout, wrap, and selection policies")
    @MainActor
    func parsedDiff() throws {
        ...
        let output = view.render(in: try AgentRenderContext(width: 30, scheme: .dark))
