...
struct SwiftTUIShowcaseSmokeTests {
    private let headings = [
        "Prompt", "Command palette", "Messages", "Reasoning", "Tools",
        "Diff", "Permissions", "Questions", "Toast", "Sidebar",
    ]
    ...
    @Test("Showcase renders, activates its command palette, and restores a PTY", .timeLimit(.minutes(1)))
    func showcasePTYSmoke() throws {
        let pty = try PTYPair(columns: 120, rows: 40)
        let executable = try testExecutableDirectory()
            .appendingPathComponent("SwiftTUIShowcase")
        let harness = try testExecutableDirectory()
            .appendingPathComponent("SwiftTUIShowcasePTYHarness")
        ...
        var output = try pty.readUntil(timeoutMilliseconds: 10_000) { bytes in
            guard let text = String(bytes: bytes, encoding: .utf8) else { return false }
            return text.contains("SwiftTUI · Agent UI Showcase")
                && text.contains("Animation state: idle")
                && headings.allSatisfy(text.contains)
        }

        try pty.writeToMaster(Array("\t\r".utf8), timeoutMilliseconds: 1_000)
        output += try pty.readUntil(timeoutMilliseconds: 5_000) { bytes in
            String(decoding: bytes, as: UTF8.self).contains("Animation state: active")
        }
        try pty.writeToMaster(Array("\t\r".utf8), timeoutMilliseconds: 1_000)
        output += try pty.readUntil(timeoutMilliseconds: 5_000) { bytes in
            String(decoding: bytes, as: UTF8.self).contains("SWIFTTUI_SHOWCASE_COMMAND=focused-tests")
        }
        try pty.writeToMaster(Array("q".utf8), timeoutMilliseconds: 1_000)
        output += try pty.readUntil(timeoutMilliseconds: 5_000) { bytes in
            String(decoding: bytes, as: UTF8.self).contains(restoration)
        }
        let exitCode = try waitForExit(processID, timeoutMilliseconds: 5_000)
        output += try pty.readRemaining(timeoutMilliseconds: 1_000)
        let text = String(decoding: output, as: UTF8.self)

        #expect(exitCode == 0)
        #expect(text.contains("SwiftTUI · Agent UI Showcase"))
        #expect(text.contains("Animation state: idle"))
        #expect(text.contains("Animation state: active"))
        #expect(text.contains("Run focused tests"))
        #expect(text.contains("SWIFTTUI_SHOWCASE_COMMAND=focused-tests"))
...
private enum SmokeError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .missingExecutable(let path): "Missing built showcase executable at \(path)"
        case .posix(let operation, let code): "\(operation) failed with errno \(code)"
        case .processExitedBeforeFrame: "Showcase exited before rendering its first frame"
        case .timedOut(let operation): "Timed out while \(operation)"
        }
    }
}
...
private final class PTYPair {
    ...
    init(columns: UInt16, rows: UInt16) throws {
        master = showcasePosixOpenpt(O_RDWR | O_NOCTTY | O_NONBLOCK)
    ...
    func readUntil(timeoutMilliseconds: Int32, condition: ([UInt8]) -> Bool) throws -> [UInt8] {
        let deadline = monotonicMilliseconds() + UInt64(timeoutMilliseconds)
        var output: [UInt8] = []
        while condition(output) == false {
            try waitForMaster(events: Int16(POLLIN), deadline: deadline, operation: "waiting for showcase frame")
    ...
    func readRemaining(timeoutMilliseconds: Int32) throws -> [UInt8] {
        ...
        while monotonicMilliseconds() < deadline {
            ...
            if result > 0 {
                if try appendAvailable(to: &output) == false { break }
    ...
    private func appendAvailable(to output: inout [UInt8]) throws -> Bool {
        ...
        if result > 0 { output.append(contentsOf: buffer.prefix(result)); return true }
        if result < 0, errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK { return true }
        if result == 0 || errno == EIO { return false }
    ...
    private func waitForMaster(events: Int16, deadline: UInt64, operation: String) throws {
        while true {
            ...
            if result > 0, descriptor.revents & events != 0 { return }
            if result == 0 { throw SmokeError.timedOut(operation) }
            if result < 0, errno == EINTR { continue }
            if result > 0, descriptor.revents & Int16(POLLHUP) != 0 { throw SmokeError.processExitedBeforeFrame }
            throw SmokeError.posix("poll", errno)
        }
    }
}
...
private func waitForExit(_ processID: pid_t, timeoutMilliseconds: Int32) throws -> Int32 {
    ...
    while monotonicMilliseconds() < deadline {
        ...
        if result < 0, errno != EINTR { throw SmokeError.posix("waitpid", errno) }
        usleep(10_000)
    }
