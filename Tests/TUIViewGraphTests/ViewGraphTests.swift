...
struct ViewGraphTests {
    ...
    @Test
    func structuralIdentityUsesPositionForUnkeyedNodesAndPreservesKeyedIdentityAcrossReorder() {
        let first = StructuralIdentity(type: Row.self, index: 0)
        let second = StructuralIdentity(type: Row.self, index: 1)
        let keyedFirst = StructuralIdentity(type: Row.self, index: 0, key: "row")
        let keyedSecond = StructuralIdentity(type: Row.self, index: 4, key: "row")

        #expect(first != second)
        #expect(keyedFirst == keyedSecond)
        #expect(keyedFirst != StructuralIdentity(type: Row.self, index: 0, key: "row", branch: 1))
    }

    @Test
    func nestedBranchIdentityPreservesTheFullPath() {
    ...
    @Test
    func keyedReorderPreservesNodesAndState() throws {
        let graph = ViewGraph()
    ...
    @Test
    func stateMutationInvalidatesStructure() throws {
        let graph = ViewGraph()
    ...
    @Test
    func updateReplacesPrimitiveWhilePreservingNodeID() throws {
        let graph = ViewGraph()
    ...
    @Test
    func rollbackRestoresFrameSnapshottingMetadataReference() throws {
        let graph = ViewGraph()
    ...
    @Test
    func presentationOnlyRemovalRetainsPrimitive() throws {
        let graph = ViewGraph()
    ...
    @Test
    func presentationRemovalRetainsDeterministicSiblingPlacement() throws {
        let graph = ViewGraph()
        ...
        #expect(removed.presentationPlacement == PresentationPlacement(
    ...
    @Test
    func staleRemovalTokenCannotDeleteReclaimedNode() throws {
        let graph = ViewGraph()
        let retained = NodeDescriptor(
            type: Row.self,
            key: "retained",
    ...
    @Test
    func invalidationHandlerReceivesOriginatingNodeAndFlagsOnce() throws {
    ...
    @Test
    func changingConditionalBranchReplacesNode() throws {
        let graph = ViewGraph()
    ...
    @Test
    func lifecycleRunsAfterCommitAndNotForDiscardedPlan() throws {
        let graph = ViewGraph()
        ...
        let discarded = try graph.prepare(NodeDescriptor(
    ...
    @Test
    func stagedCommitRollsBackGraphMutationAndLifecycle() throws {
        let graph = ViewGraph()
        let stateKey = StateKey<Int>(CounterState.self)
        var updateCount = 0
        var mountCount = 0
    ...
    @Test
    func stagedCommitFinalizesRevisionAndLifecycleOnlyOnFinish() throws {
        let graph = ViewGraph()
        var mountCount = 0
        let descriptor = NodeDescriptor(
            type: Root.self,
    ...
    @Test
    func lifecycleRejectsReentrantCommitWithTypedError() throws {
        let graph = ViewGraph()
    ...
    @Test
    func planDoesNotOutliveItsGraphUnsafely() throws {
    ...
    @Test
    func dirtyFlagsCoalesceAndPropagateToAncestors() throws {
        let graph = ViewGraph()
        try graph.commit(graph.prepare(root(children: [row("a")])))
        graph.clearDirtyFlags()
        let child = try #require(graph.root?.children.first)
    ...
    @Test
    func incrementalFrameTracksLocalAndAggregateDirtyState() throws {
        let graph = ViewGraph()
        try graph.commit(graph.prepare(root(children: [row("a")])))
        graph.clearDirtyFlags()
        let root = try #require(graph.root)
    ...
    @Test
    func incrementalFrameRollsBackGeometryMetadataAndDirtyState() throws {
        let graph = ViewGraph()
        try graph.commit(graph.prepare(root(children: [row("a")])))
        graph.clearDirtyFlags()
        let child = try #require(graph.root?.children.first)
    ...
    @Test
    func incrementalFrameClearsOnlyUnchangedDirtyGenerations() throws {
        let graph = ViewGraph()
        try graph.commit(graph.prepare(root(children: [row("a")])))
        graph.clearDirtyFlags()
        let child = try #require(graph.root?.children.first)
    ...
    @Test
    func retainedNodeDetachesFromFocusAndHitTestingUntilTransitionCompletes() throws {
        let graph = ViewGraph()
    ...
    @Test
    func removalDetachesImmediatelyByDefault() throws {
        let graph = ViewGraph()
    ...
    @Test
    func retainedParentDoesNotRetainDefaultChildren() throws {
        let graph = ViewGraph()
        let descriptor = NodeDescriptor(
            type: Root.self,
            children: [row("child")],
    ...
    @Test
    func retainedNodeReinsertionPreservesIdentityAndState() throws {
        let graph = ViewGraph()
        let stateKey = StateKey<Int>(CounterState.self)
    ...
    @Test
    func duplicateKeyProducesTypedDiagnostic() throws {
        let graph = ViewGraph()
