...
struct LayoutTests {
    ...
    @Test("Declarative stacks retain indexed children and layout configuration")
    func declarativeStacks() throws {
        let vertical = try #require(VStack(alignment: .trailing, spacing: 2) {
            NodeDescriptor(type: Leaf.self, value: 1)
            NodeDescriptor(type: Leaf.self, value: 2)
        }.graphBody.first)
        let horizontal = try #require(HStack(alignment: .bottom, spacing: 3) {
        ...
        #expect(vertical.value(as: LayoutPrimitive.self) == .stack(
        ...
        #expect(horizontal.primitive(as: LayoutPrimitive.self) == .stack(
            StackLayout(axis: .horizontal, spacing: 3, verticalAlignment: .bottom)
        ))
    }

    @Test("Layout modifiers wrap content and retain snapshots")
    func declarativeModifiers() throws {
        ...
        let layoutNodes: [(MountedNode, LayoutPrimitive)] = descendants(of: try #require(graph.root)).compactMap { node in
            node.primitive(as: LayoutPrimitive.self).map { (node, $0) }
        }
        let descriptor = try #require(layoutNodes.first { _, primitive in
            primitive == LayoutPrimitive.frame(FrameLayout(width: 12, height: 8, alignment: .bottomTrailing))
        }?.0)
        let padding = try #require(layoutNodes.first { _, primitive in
            primitive == LayoutPrimitive.padding(
                PaddingLayout(EdgeInsets(top: 1, leading: 2, bottom: 3, trailing: 4))
            )
        }?.0)

        #expect(descriptor.children.count == 1)
        #expect(descriptor.primitive(as: LayoutPrimitive.self) == .frame(
        ...
        #expect(padding.primitive(as: LayoutPrimitive.self) == .padding(
            PaddingLayout(EdgeInsets(top: 1, leading: 2, bottom: 3, trailing: 4))
        ))
    }

    @Test("Declarative primitive configuration drives existing layout algorithms")
    func declarativePrimitiveGeometry() throws {
        let descriptor = try #require(VStack(alignment: .trailing, spacing: 1) {
            NodeDescriptor(type: Leaf.self)
            NodeDescriptor(type: Leaf.self)
        }.graphBody.first)
        ...
        #expect(result.placements.map(\.frame) == [
            CellRect(x: 2, y: 0, width: 2, height: 1),
            CellRect(x: 0, y: 2, width: 4, height: 2),
        ])
    }

    @Test("Vertical stack measures and aligns children")
    func verticalStack() {
    ...
    @Test("Stack accepts representable geometry at the Int boundary")
    func stackGeometryAtIntBoundary() {
        let items = [
    ...
    @Test("Overlay uses both alignment axes")
    func overlayAlignment() {
        let cache = LayoutCache()
    ...
    @Test("Cache isolates measurements by node and proposal")
    func cacheLocality() {
        let cache = LayoutCache()
    ...
    @Test("Mounted node layout invalidation advances the cache generation")
    func mountedNodeCacheInvalidation() throws {
        enum Root { }
        let graph = ViewGraph()
    ...
    @Test("Frame constrains a fixed axis before measuring the other axis")
    func frameMeasurementOrder() {
        let item = LayoutItem(nodeID: NodeID(rawValue: 1)) { proposal in
    ...
    @Test("Disjoint nested clips produce an empty clip")
    func disjointClips() {
        ...
        let nested = clipped(result, to: CellRect(x: 5, y: 0, width: 2, height: 1))

        #expect(nested.placements[0].clip?.isEmpty == true)
    }

    @Test("Lazy plan instantiates only a bounded visible range")
    func lazyRange() {
        let plan = LazyLayoutPlanner(itemExtent: 1, overscan: 2)
            .plan(itemCount: 10_000, viewport: 5_000..<5_040)
        #expect(plan.visibleRange == 4_998..<5_042)
        #expect(plan.visibleRange.count == 44)
        #expect(plan.contentExtent == 10_000)
    }

    @Test("Lazy plan returns empty ranges for empty and out-of-content viewports")
    func lazyEmptyRanges() {
        ...
        #expect(LazyLayoutPlanner(itemExtent: 1, spacing: 1)
            .plan(itemCount: 10, viewport: 19..<19).visibleRange == 10..<10)
        #expect(planner.plan(itemCount: 0, viewport: 20..<30).visibleRange == 0..<0)
    }

    @Test("Prepend correction preserves the visual anchor")
    func prependAnchor() {
        var state = ScrollState(offset: 20, viewportExtent: 10, contentExtent: 100)
        state.contentExtent = 105
        state.preserveAnchor(oldOrigin: 20, newOrigin: 25)
        #expect(state.offset == 25)
    }

    @Test("Bottom anchoring stops after the user scrolls away")
    func anchoredBottom() {
        var pinned = ScrollState(offset: 90, viewportExtent: 10, contentExtent: 100)
        pinned.updateContentExtent(120, anchorToBottom: true)
        #expect(pinned.offset == 110)

        var detached = ScrollState(offset: 80, viewportExtent: 10, contentExtent: 100)
