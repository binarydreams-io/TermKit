...
struct PrimitivesTests {
    ...
    func surfaceLeafDescriptorAndSizing() throws {
        ...
        #expect(descriptor.primitive(as: Surface.self) == surface)
    ...
    func activityViewAdapter() throws {
        let descriptor = try #require(ActivityIndicatorView(frames: ["a", "b"], interval: .milliseconds(100))
            .graphBody.first)
    ...
    func dialogWidths() {
        let dialog = DialogSurface(id: "preferences", title: "Preferences", preferredWidth: .wide, content: "body")
    ...
    func surfaceElevationPaint() throws {
        ...
        var flatSurface = TUIRenderer.Surface(size: bounds.size)
        var raisedSurface = TUIRenderer.Surface(size: bounds.size)
        _ = try Surface(text: "A", foreground: .white, background: .black, elevation: .flat)
            .paint(into: &flatSurface, context: PaintContext(clip: bounds), resources: &resources)
        _ = try Surface(text: "A", foreground: .white, background: .black, elevation: .raised)
    ...
    func dialogPaint() throws {
        var surface = TUIRenderer.Surface(size: CellSize(width: 70, height: 7))
        var resources = ControlRenderResources()
        let dialog = DialogSurface(id: "confirm", title: "Confirm", preferredWidth: .compact, content: "Proceed?")
    ...
    func toastPaint() throws {
        var surface = TUIRenderer.Surface(size: CellSize(width: 20, height: 4))
        var resources = ControlRenderResources()
        let toast = Toast(id: "saved", message: "Saved", kind: .success, createdAt: .zero)
