...
struct InternerPropertyTests {
    @Test func boundedInternerRebuildsPreserveRetainedValuesAndRemapIdentifiers() {
        runInternerPropertyCases(count: 192, baseSeed: 0x1A7E_AE12_5EED_0001) { random in
...
private func runInternerPropertyCases(
    ...
    for caseIndex in 0..<count {
        let seed = baseSeed &+ UInt64(caseIndex) &* 0x9E37_79B9_7F4A_7C15
...
private func checkGraphemeInterner(random: inout InternerTestRandom) throws {
    ...
    for value in internerGraphemeFixtures.shuffled(using: &random) {
        entries.append((try interner.intern(value), value))
        if interner.stats.requiresRebuild { break }
    }
...
private func checkStyleInterner(random: inout InternerTestRandom) throws {
    ...
    for index in 0..<32 {
        ...
        entries.append((try interner.intern(style), style))
        if interner.stats.requiresRebuild { break }
    }
