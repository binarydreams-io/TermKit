...
struct TUIRuntimeTests {
    ...
    func boundAgentPromptRuntimeIntegration() throws {
        ...
        let second = try #require(try runtime.renderIfDue(
    ...
    func agentPromptMetadataInsertionTransition() throws {
        ...
        let metadataNode = try #require(firstNode(runtime.graph.root) {
        ...
        let sampledMidpoint = try #require(metadataNode.presentationValue(
        ...
        #expect(firstNode(staticRuntime.graph.root) {
            $0.animationStatus(for: .transitionVisibility) != nil
        } == nil)
    ...
    func agentAdaptersRuntimeIntegration() throws {
        ...
        let viewportRuntime = TUIRuntime(
            view: AgentComponentView(
                items: ["one", "two", "three"],
        ...
        let autocomplete = PromptAutocomplete(state: PromptAutocompleteState(
            suggestions: [
        ...
        let autocompleteRuntime = TUIRuntime(
            view: autocomplete,
            presenter: FramePresenter(session: FakeTerminalSession()),
        ...
        try autocompleteRuntime.process(.input(.mouse(TerminalMouseEvent(
            action: .release(.left),
    ...
    func declarativeRuntimeIntegration() throws {
        ...
        let third = try #require(try runtime.renderIfDue(
    ...
    func stateMutationAnimationTransaction() throws {
        ...
        let animatedNode = try #require(firstNode(runtime.graph.root) {
    ...
    func concurrentInvalidationChannel() async throws {
        ...
        let result = try #require(try runtime.renderIfDue(
            at: .zero.advanced(by: FrameScheduler.minimumFrameInterval)
        ))
    ...
    func reducedMotionTimelineIsStatic() throws {
        ...
        let runtime = TUIRuntime(
            view: TimelineView(.animation()) { context in
                Text(context.reduceMotion ? "static" : "moving")
            },
            presenter: presenter,
    ...
    func reducedMotionIsFinalAndStatic() throws {
        ...
        #expect(view.transactions.last?.animationsEnabled == true)
        #expect(view.transactions.last?.reduceMotion == true)
        #expect(runtime.nextDeadline(at: .zero) == nil)
    }
...
@MainActor
private struct RuntimePromptTransitionRoot: TUIViewGraph.View {
    let model: RuntimePromptTransitionModel
...
@MainActor
private struct RuntimeAgentComponentRoot: TUIViewGraph.View {
    let model: RuntimeAgentComponentModel
...
@MainActor
private struct RuntimePermissionRoot: TUIViewGraph.View {
    let model: RuntimePermissionModel
...
@MainActor
private struct RuntimeQuestionRoot: TUIViewGraph.View {
    let model: RuntimeQuestionModel
...
@MainActor
private struct DeclarativeTestRoot: TUIViewGraph.View {
    ...
    var graphBody: [NodeDescriptor] {
        buildViewGraph {
            VStack(alignment: .leading) {
                ...
                AgentComponentView(
                    ToastPresentation(kind: .information, message: "Agent"),
...
private enum RuntimePaintValueKey: TUIViewGraph.EnvironmentKey, PaintEnvironmentKey {
    static let defaultValue = "D"
}

@MainActor
private struct PaintEnvironmentRoot: TUIViewGraph.View {
    var graphBody: [NodeDescriptor] {
        HStack(spacing: 0) {
            EnvironmentPaintView()
            EnvironmentPaintView().environment(RuntimePaintValueKey.self, "O")
        }
        .environment(RuntimePaintValueKey.self, "P")
        .graphBody
    }
}

@MainActor
private struct EnvironmentPaintView: TUIViewGraph.View {
    var graphBody: [NodeDescriptor] {
...
@MainActor
private struct AnimatedStateRoot: TUIViewGraph.View {
...
private final class FakeRuntimeEventSource: RuntimeEventSource, @unchecked Sendable {
    ...
    private var recordedTimeouts: [TUIDuration?] = []
...
private final class MutableRuntimeTimeSource: TimeSource, @unchecked Sendable {
    ...
    func advance(by duration: TUIDuration) {
...
private final class TestRuntimeView: RuntimeView {
    ...
    func paint(
        ...
        var surface = TUIRenderer.Surface(size: context.terminalSize)
