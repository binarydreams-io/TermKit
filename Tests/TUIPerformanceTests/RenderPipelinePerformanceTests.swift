...
@Suite(
    ...
    .disabled(
        if: swiftTUIPerformanceTestsEnabled == false,
        "Set SWIFTTUI_RUN_PERFORMANCE_TESTS=1 and use a release build."
    )
)
struct RenderPipelinePerformanceTests {
    ...
    func localizedAnimation() throws {
        ...
        let result = try measureBenchmark(
            ...
            inspect: { frame in
                contractIsValid = contractIsValid
    ...
    func declarativePaintAnimation() throws {
        ...
        let inspect: (RuntimeFrameResult) -> Void = { frame in
            let phasesAreIncremental = frame.stats.reconciliationDuration == .zero
            ...
            let damageIsBounded = frame.presentation.stats.scannedCellCount > 0
                && frame.presentation.stats.scannedCellCount <= size.width
                && frame.presentation.stats.changedCellCount <= size.width
            let semanticsAreComplete = frame.semantics.node(withID: "row-0") != nil
...
@MainActor
private final class DeclarativePaintBenchmarkView: TUIViewGraph.View {
