...
let swiftTUIPerformanceTestsEnabled =
    ProcessInfo.processInfo.environment["SWIFTTUI_RUN_PERFORMANCE_TESTS"] == "1"
...
final class RuntimeFrameHarness {
    ...
    init(size: CellSize) throws {
        self.size = size
        session = BenchmarkTerminalSession()
        let presenter = FramePresenter(session: session)
        view = try BenchmarkRuntimeView(size: size, presenter: presenter)
        runtime = TUIRuntime(
            view: view,
            presenter: presenter,
            terminalSize: size,
...
final class BenchmarkRuntimeView: RuntimeView {
    ...
    init(size: CellSize, presenter: FramePresenter) throws {
        primaryGrapheme = try presenter.withRenderResources { try $0.graphemes.intern("a") }
        alternateGrapheme = try presenter.withRenderResources { try $0.graphemes.intern("b") }
        surface = Surface(
            size: size,
