...
@MainActor
struct SwiftTUIIntegrationTests {
    private enum Root {}

    @Test("Umbrella product exposes declarative views and animations")
    func publicSurface() {
        ...
        #expect(ShowcaseCatalog().missingRequiredComponents.isEmpty)
        #expect(SwiftTUIRelease.version == "0.1.0-preview")
    }

    @Test("Generic runtime view initializer is public")
    func genericRuntimeInitializer() {
    ...
    @Test("Reduced motion exposes a static activity indicator")
    func reducedMotion() {
    ...
    @Test("Umbrella product composes a bound agent prompt with accessories")
    func boundAgentPromptComposition() {
        final class Model {
    ...
    @Test("Public documentation examples compile")
    func documentationExamples() {
...
private final class CompileOnlyRuntimeSession: RuntimeTerminalSession {
    ...
    func start() throws -> TerminalSessionTransition { state = .active; return .started }
    func suspend() throws -> TerminalSessionTransition { state = .suspended; return .suspended }
    func resume() throws -> TerminalSessionTransition { state = .active; return .resumed(requiresFullRepaint: true) }
    func stop() throws -> TerminalSessionTransition { state = .inactive; return .stopped }
    func present(_ bytes: [UInt8]) throws {}
    func handleSignalEvent(_ event: TerminalSignalEvent) throws -> TerminalSignalAction { .terminate }
    func readInput(maximumByteCount: Int) throws -> [UInt8] { [] }
    func readTerminalSize() throws -> TerminalSize { TerminalSize(columns: 10, rows: 2) }
}
