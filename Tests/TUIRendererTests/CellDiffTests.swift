...
struct CellDiffTests {
    @Test func replayingDiffProducesBackSurface() throws {
        var front = Surface(size: CellSize(width: 6, height: 2))
        var back = front
        try front.write(graphemeID: GraphemeID(rawValue: 1), displayWidth: 2, at: CellPoint(x: 1, y: 0))
        try back.write(graphemeID: GraphemeID(rawValue: 2), styleID: StyleID(rawValue: 1), at: CellPoint(x: 0, y: 0))
        try back.write(
    ...
    @Test func diffRewritesShortUnchangedGap() throws {
        ...
        let writes = result.operations.compactMap { operation -> GraphemeID? in
            guard case .write(let identifier, _, _) = operation else { return nil }
            return identifier
        }

        #expect(writes == [GraphemeID(rawValue: 1), .space, GraphemeID(rawValue: 2)])
        #expect(result.operations.filter { if case .moveCursor = $0 { true } else { false } }.count == 1)
    }

    @Test func unchangedSurfaceProducesNoOperations() throws {
