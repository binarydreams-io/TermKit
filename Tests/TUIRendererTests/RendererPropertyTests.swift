...
private func runRendererPropertyCases(
    ...
    let configuredSeed = ProcessInfo.processInfo.environment["SWIFTTUI_FUZZ_SEED"].flatMap { value -> UInt64? in
        let digits = value.hasPrefix("0x") ? String(value.dropFirst(2)) : value
        return UInt64(digits, radix: value.hasPrefix("0x") ? 16 : 10)
    }
    let seeds = configuredSeed.map { [$0] } ?? (0..<count).map {
    ...
    for (caseIndex, seed) in seeds.enumerated() {
        ...
        do {
            ...
            let diagnostic = "Property case \(caseIndex) failed with seed 0x\(seedText): \(error). "
                + "Reproduce with SWIFTTUI_FUZZ_SEED=0x\(seedText) "
