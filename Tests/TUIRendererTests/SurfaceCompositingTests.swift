...
struct SurfaceCompositingTests {
    ...
    @Test func nestedTransparentCompositingPreservesAlphaUntilOpaqueDestination() throws {
        ...
        try destination.write(graphemeID: lowerGlyph, styleID: red, at: .zero)
        try child.write(graphemeID: upperGlyph, styleID: white, at: .zero)
    ...
    @Test func halfOpacityBlendsWhiteOverBlackInLinearSRGB() throws {
        ...
        try destination.write(graphemeID: lowerGrapheme, styleID: lower, at: .zero)
        try source.write(graphemeID: upperGrapheme, styleID: upper, at: .zero)
    ...
    @Test func zeroOpacityPreservesDestinationAndOneUsesSource() throws {
        ...
        var destination = Surface(size: CellSize(width: 1, height: 1))
        var source = Surface(size: CellSize(width: 1, height: 1))
        try destination.write(graphemeID: lowerGrapheme, styleID: lowerStyle, at: .zero)
        try source.write(graphemeID: upperGrapheme, styleID: upperStyle, at: .zero)
        ...
        try destination.composite(
            ...
            graphemes: graphemes,
            styles: &styles,
            resolveColor: { _, _ in Issue.record("Opacity zero resolved a color"); return .clear }
        )
    ...
    @Test func clipUsesActualLowerLayerStyles() throws {
        ...
        try destination.write(graphemeID: lowerGrapheme, styleID: firstLower, at: .zero)
        try destination.write(graphemeID: lowerGrapheme, styleID: secondLower, at: CellPoint(x: 1, y: 0))
        try source.write(graphemeID: upperGrapheme, styleID: upper, at: .zero)
        try source.write(graphemeID: upperGrapheme, styleID: upper, at: CellPoint(x: 1, y: 0))
    ...
    @Test func wideSourceAndDestinationAtomsDoNotCrossClipBoundaries() throws {
        ...
        try destination.write(graphemeID: narrow, styleID: style, at: .zero)
        try destination.write(graphemeID: wide, styleID: style, displayWidth: 2, at: CellPoint(x: 1, y: 0))
        try wideSource.write(graphemeID: wide, styleID: style, displayWidth: 2, at: .zero)
        let original = destination
        ...
        try narrowSource.write(graphemeID: narrow, styleID: style, at: .zero)
        try destination.composite(
            narrowSource,
            at: CellPoint(x: 2, y: 0),
            clip: CellRect(x: 2, y: 0, width: 1, height: 1),
