...
struct FrameModifierTests {
  ...
  @Test("Frame alignment translates interaction regions")
  func frameAlignmentTranslatesRegions() {
    ...
    let buffer = frame.renderToBuffer(context: testContext())

    #expect(buffer.regions == [
      InteractionRegion(id: "content", rect: TerminalCellRect(x: 3, y: 1, width: 2, height: 1))
    ])
  }
}
