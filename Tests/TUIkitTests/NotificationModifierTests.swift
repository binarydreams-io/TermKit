...
struct NotificationTests {
    ...
    @Test("A rail surface replaces the box with a leading accent rail")
    func railSurfaceDrawsNoBorder() {
        let context = testContext()
        let service = NotificationService()
        service.post("Copied")
        var env = context.environment
        ...
        let view = Text("Base").notificationHost(
            width: 24,
            surface: .rail(accent: .green, background: .black)
        )

        let buffer = renderToBuffer(view, context: context.withEnvironment(env))
        let joined = buffer.lines.joined()

        #expect(joined.contains("Copied"))
        #expect(joined.contains("┃"))
        for glyph in "╭╮╰╯┌┐└┘─│" {
            #expect(!joined.contains(glyph))
        }
    }
}
