...
struct ButtonRowTests {
  ...
  @Test("ButtonRow preserves translated regions and actions")
  func buttonRowPreservesInteractions() throws {
    ...
    let context = RenderContext(
      ...
      availableHeight: 10,
      tuiContext: tuiContext,
      identity: ViewIdentity(path: "button-row")
    )
    let row = ButtonRow(spacing: 3) {
      Button("First") { actions.append("first") }
      Button("Second") { actions.append("second") }
    }

    let buffer = renderToBuffer(row, context: context)
    let regions = buffer.regions.sorted { $0.rect.x < $1.rect.x }
    let first = try #require(regions.first)
    let second = try #require(regions.last)

    #expect(regions.count == 2)
    #expect(Set(regions.map(\.id)).count == 2)
    #expect(first.rect.x == 40 - first.rect.width - 3 - second.rect.width)
    #expect(second.rect.x == first.rect.maxX + 3)
