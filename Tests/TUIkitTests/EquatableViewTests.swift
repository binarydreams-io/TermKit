...
private struct CachedButtonView: View {
  let label: String
  let action: () -> Void
...
extension CachedButtonView: @preconcurrency Equatable {
  static func == (lhs: Self, rhs: Self) -> Bool {
    lhs.label == rhs.label
  }
}
...
struct EquatableViewTests {
  ...
  @Test("Interactive subtrees bypass the cache and restore click registrations")
  func interactiveSubtreeBypassesCache() throws {
    let tuiContext = TUIContext()
    let context = RenderContext(
      availableWidth: 80,
      availableHeight: 24,
      environment: tuiContext.environmentValues(),
      identity: ViewIdentity(path: "Root/InteractiveCache")
    )
    var clickCount = 0
    let view = EquatableView(
      content: CachedButtonView(label: "Click") { clickCount += 1 }
    )
    ...
    tuiContext.beginRenderPass()
    _ = renderToBuffer(view, context: measurementContext)
    tuiContext.endRenderPass()
    #expect(tuiContext.renderCache.count == 0)
    ...
    #expect(tuiContext.renderCache.count == 0)
    let region = try #require(buffer.regions.first)
