...
struct FocusSectionTests {
    ...
    @Test("Activating the active section preserves focus without notification")
    func activatingActiveSectionIsNoOp() {
        let manager = FocusManager()
        manager.registerSection(id: "panel")

        let first = MockFocusable(id: "first")
        let second = MockFocusable(id: "second")
        manager.register(first, inSection: "panel")
        manager.register(second, inSection: "panel")
        manager.focus(second)

        let firstLostCount = first.focusLostCount
        let secondReceivedCount = second.focusReceivedCount
        let secondLostCount = second.focusLostCount
        var focusChangeCount = 0
        manager.onFocusChange = { focusChangeCount += 1 }

        manager.activateSection(id: "panel")
        manager.activateSection(id: "panel")

        #expect(manager.isFocused(second))
        #expect(first.focusLostCount == firstLostCount)
        #expect(second.focusReceivedCount == secondReceivedCount)
        #expect(second.focusLostCount == secondLostCount)
