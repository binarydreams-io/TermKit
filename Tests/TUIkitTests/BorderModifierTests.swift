...
struct BorderModifierTests {
  ...
  @Test(".border() translates interaction regions past padding and border cells")
  func borderModifierTranslatesRegions() {
    let content = BufferView(
      buffer: FrameBuffer(
        text: "Hi",
        regions: [InteractionRegion(id: "content", rect: TerminalCellRect(x: 0, y: 0, width: 2, height: 1))]
      )
    )

    let buffer = renderToBuffer(content.border(.line), context: testContext())

    #expect(buffer.regions == [
      InteractionRegion(id: "content", rect: TerminalCellRect(x: 2, y: 1, width: 2, height: 1))
    ])
  }
}
