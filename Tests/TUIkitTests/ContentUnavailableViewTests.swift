...
struct ContentUnavailableViewTests {
  ...
  @Test("Actions retain centered clickable regions")
  func actionRegionsAreCenteredAndClickable() throws {
    ...
    let context = RenderContext(
      ...
      identity: ViewIdentity(path: "content-unavailable")
    )
    let view = ContentUnavailableView {
      Text("Empty")
    } description: {
      ...
      Button("Retry") { clickCount += 1 }
    }

    let buffer = renderToBuffer(view, context: context)
    let region = try #require(buffer.regions.first)
    let visibleLine = buffer.lines[region.rect.y].stripped
    let leadingSpaces = visibleLine.prefix { $0 == " " }.count

    #expect(buffer.regions.count == 1)
    #expect(region.rect.x == leadingSpaces)
