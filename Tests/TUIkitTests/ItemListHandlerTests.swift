...
struct ItemListHandlerNavigationTests {
    ...
    @Test("Down arrow requests a visual refresh")
    func moveDownRequestsVisualRefresh() {
        var refreshCount = 0
        let handler = ItemListHandler<String>(
            focusID: "test",
            itemCount: 5,
...
struct ItemListHandlerScrollTests {
    ...
    @Test("Mouse wheel movement counts selectable rows and clamps")
    func mouseWheelMovement() {
        let handler = ItemListHandler<String>(
            focusID: "test",
            itemCount: 7,
            viewportHeight: 3,
        ...
        handler.itemIDs = [nil, "a", nil, "b", "c", nil, "d"]
        handler.selectableIndices = [1, 3, 4, 6]
        handler.focusedIndex = 1

        #expect(handler.moveFocusBySelectableRows(by: 3))
        #expect(handler.focusedIndex == 6)
        #expect(handler.scrollOffset == 4)
        #expect(handler.moveFocusBySelectableRows(by: 3) == false)
        #expect(handler.focusedIndex == 6)
    ...
    @Test("Variable-height rows keep mouse wheel focus visible")
    func variableHeightMouseWheelVisibility() {
        let handler = ItemListHandler<String>(
            focusID: "test",
            itemCount: 6,
            viewportHeight: 4,
        ...
        handler.itemIDs = ["a", "b", "c", "d", "e", "f"]
        handler.rowHeights = [4, 1, 1, 1, 1, 1]
