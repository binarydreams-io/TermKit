...
struct BackgroundModifierTests {
  ...
  @Test("Background modifier preserves interaction regions")
  func backgroundPreservesRegions() {
    let region = InteractionRegion(
      id: "action",
      rect: TerminalCellRect(x: 1, y: 0, width: 3, height: 1)
    )
    let buffer = FrameBuffer(lines: [" Test"], regions: [region])
