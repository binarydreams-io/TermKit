...
struct OverlayModifierTests {
  ...
  @Test("Base and overlay use distinct interaction identities")
  func distinctInteractionIdentities() {
    ...
    let context = RenderContext(
      ...
      availableHeight: 24,
      tuiContext: tuiContext,
      identity: ViewIdentity(path: "overlay")
    )
    let view = OverlayModifier(
      base: Button("Base") {},
    ...
    #expect(buffer.regions.count == 2)
    #expect(Set(buffer.regions.map(\.id)).count == 2)
  }
}
