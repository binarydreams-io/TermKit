...
public struct TerminalInputParser: Sendable {
    ...
    public init(configuration: TerminalInputParserConfiguration = TerminalInputParserConfiguration()) {
        ...
        buffer.reserveCapacity(min(configuration.maximumSequenceByteCount, 4_096))
    }
    ...
    private mutating func parseSS3(into output: inout TerminalInputParserOutput) {
        ...
        offset += 3
        let key: TerminalKey? = switch sequence[2] {
        case 0x41: .up
        case 0x42: .down
        case 0x43: .right
        case 0x44: .left
...
extension TerminalInputParser {
    private func parseCSISequence(_ sequence: [UInt8]) -> TerminalInputEvent? {
        ...
        let key: TerminalKey? = switch finalByte {
        case 0x41: .up
        case 0x42: .down
        case 0x43: .right
        case 0x44: .left
    ...
    private func mouseAction(code: Int, finalByte: UInt8) -> TerminalMouseAction? {
        ...
        if code & 64 != 0 {
            let direction: TerminalScrollDirection = switch buttonCode {
            case 0: .up
            case 1: .down
            case 2: .left
            default: .right
    ...
    private func parseKittyKey(_ body: ArraySlice<UInt8>) -> TerminalKeyEvent? {
        ...
        let action: TerminalKeyAction = switch modifierParts.count > 1 ? parseInteger(modifierParts[1]) : 1 {
        case 2: .repeat
        case 3: .release
        default: .press
        }
    ...
    private func kittyKey(codePoint: Int) -> TerminalKey? {
        switch codePoint {
        case 13: .enter
        case 27, 57_344: .escape
        case 9, 57_346: .tab
        case 8, 127, 57_347: .backspace
        case 57_345: .enter
        case 57_348: .insert
        case 57_349: .delete
        case 57_350: .left
        ...
        case 57_353: .down
        case 57_354: .pageUp
        case 57_355: .pageDown
        case 57_356: .home
        case 57_357: .end
        case 57_364 ... 57_398: .function(codePoint - 57_363)
        default:
    ...
    private func tildeKey(_ parameter: Int??) -> TerminalKey? {
        guard case .some(.some(let parameter)) = parameter else { return nil }
    ...
    private func modifiers(fromBitField value: Int) -> TerminalKeyModifiers {
        ...
        if value & 1 != 0 { result.insert(.shift) }
        if value & 2 != 0 { result.insert(.alt) }
        if value & 4 != 0 { result.insert(.control) }
        if value & 8 != 0 { result.insert(.super) }
        if value & 16 != 0 { result.insert(.hyper) }
        if value & 32 != 0 { result.insert(.meta) }
        if value & 64 != 0 { result.insert(.capsLock) }
        if value & 128 != 0 { result.insert(.numLock) }
        return result
    }
}
...
extension TerminalInputParser {
    ...
    private mutating func compactConsumedBytes() {
        ...
        if offset == buffer.count {
            buffer.removeAll(keepingCapacity: true)
            offset = 0
        } else if offset > 4_096, offset > buffer.count / 2 {
            buffer = Array(buffer.dropFirst(offset))
            offset = 0
        }
    }
