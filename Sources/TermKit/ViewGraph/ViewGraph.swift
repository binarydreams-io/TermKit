...
public final class ReconciliationPlan {
  ...
  fileprivate init(
    ...
    self.insertionCount = counts.insertions
    self.updateCount = counts.updates
    self.removalCount = counts.removals
...
public final class ViewGraph {
  ...
  public init(invalidationHandler: InvalidationHandler? = nil) {
    self.root = nil
    self.presentationRoots = []
    self.revision = 0
    self.environment = EnvironmentValues()
    self.invalidationHandler = invalidationHandler
    self.nextNodeID = 1
    self.activeNodes = [:]
    self.activeCommit = nil
    self.activeFrame = nil
    self.isRunningLifecycle = false
  ...
  public func prepare(_ view: some View) throws -> ReconciliationPlan {
  ...
  public func beginCommit(_ plan: ReconciliationPlan) throws -> ViewGraphCommit {
    ...
    let previousNodeIDs = Set(snapshot.nodes.map(\.node.id))
  ...
  private func normalize(_ descriptor: NodeDescriptor) throws -> NodeDescriptor {
    ...
    for (index, child) in descriptor.children.enumerated() {
      ...
      try children.append(normalize(indexedChild))
    }
  ...
  private func allMountedNodes() -> [MountedNode] {
    ...
    for root in [root].compactMap(\.self) + presentationRoots {
