...
public struct TerminalEnvironment: Equatable, Sendable {
  ...
  public init(values: [String: String], maximumValueByteCount: Int = 4096) {
    let limit = max(1, min(maximumValueByteCount, 65536))
  ...
  ///
  /// - Complexity: O(n), where n is the total size of recognized values.
  public static func makeCurrent(maximumValueByteCount: Int = 4096) -> Self {
    let limit = max(1, min(maximumValueByteCount, 65536))
...
public struct StaticTerminfoHintProvider: TerminfoHintProvider, Sendable {
  ...
  /// Returns the caller-provided record.
...
public struct InfocmpTerminfoHintProvider: TerminfoHintProvider, Sendable {
  ...
  public init(
    ...
    maximumRecordByteCount: Int = 16384,
    timeoutMilliseconds: Int32 = defaultTimeoutMilliseconds
  ) {
    self.executablePath = executablePath
    self.maximumRecordByteCount = min(max(maximumRecordByteCount, 64), 65536)
    self.timeoutMilliseconds = min(max(timeoutMilliseconds, 10), 1000)
  }

  /// Runs `infocmp` and returns its bounded output.
  public func capabilityRecord(for terminalName: String) -> [UInt8]? {
    ...
    var output: [UInt8] = []
    output.reserveCapacity(min(maximumRecordByteCount, 4096))
    ...
    while monotonicMilliseconds() < deadline {
      ...
      var buffer = [UInt8](repeating: 0, count: min(4096, maximumRecordByteCount + 1 - output.count))
...
public struct TerminfoHintPolicy: Equatable, Sendable {
  ...
  public init(
    ...
    maximumFieldByteCount: Int = 1024
  ) {
    self.maximumRecordByteCount = min(max(maximumRecordByteCount, 64), 65536)
    self.maximumFieldCount = min(max(maximumFieldCount, 8), 1024)
    self.maximumFieldByteCount = min(max(maximumFieldByteCount, 8), 4096)
  }
}
...
public struct TerminfoHints: Equatable, Sendable {
  ...
  /// Creates a set of terminfo hints.
  public init(
...
public struct TerminfoHintParseResult: Equatable, Sendable {
  ...
  /// Creates a terminfo parse result.
  public init(hints: TerminfoHints, diagnostic: TerminfoHintDiagnostic?) {
    self.hints = hints
...
public struct TerminfoHintParser: Sendable {
  ...
  ///
  /// - Complexity: O(n), where n is the record byte count.
...
public struct TerminalCapabilityDetection: Equatable, Sendable {
  ...
  /// Creates a capability detection result.
  public init(capabilities: TerminalCapabilities, terminfoDiagnostic: TerminfoHintDiagnostic?) {
    self.capabilities = capabilities
    self.terminfoDiagnostic = terminfoDiagnostic
  }
}
...
public enum TerminalCapabilityDetector {
  ...
  public static func detection(
    ...
    let color: TerminalColorCapability = if isDumb {
      .monochrome
...
public struct TerminalProbePolicy: Equatable, Sendable {
  ...
  public init(
    ...
    self.maximumResponseByteCount = min(max(maximumResponseByteCount, 32), 4096)
...
public struct SynchronizedOutputQueryParser: Sendable {
  ...
  ///
  /// - Complexity: O(n), where n is the accumulated response size.
