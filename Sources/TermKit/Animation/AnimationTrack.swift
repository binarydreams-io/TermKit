...
public struct AnimationTrack<Value: VectorArithmetic> {
  ...
  private init(
    ...
    self.animation = transaction.areAnimationsEnabled ? transaction.animation : nil
    self.completion = transaction.completion
    self.completionParticipant = transaction.completionGroup?.register()
    ...
    if animation == nil || startValue == targetValue {
      ...
      self.velocity = Self.velocity(from: startValue, to: targetValue, animation: animation)
      self.status = .running
    }
...
private final class ConcreteAnimationTrackBox<Value: VectorArithmetic>: AnyAnimationTrackBox {
  ...
  var status: AnimationTrackStatus {
    track.status
  }
...
public final class AnimationTrackStore: FrameSnapshottingNodeMetadata {
  ...
  func setTargetDeferringActions<Value: VectorArithmetic>(
    ...
    let actions = [cancellation, result.completion].compactMap(\.self)
