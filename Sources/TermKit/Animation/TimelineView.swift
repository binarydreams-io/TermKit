...
private struct TimelineFrameAttribute: MountedFrameDemandAttribute {
  ...
  var id: AnyHashable {
    ObjectIdentifier(TimelineFrameAttributeID.self)
  }
  ...
  func frameDemand(after instant: TimeInstant) -> MountedFrameDemand? {
    ...
    case .animation:
      schedule.cadence.map { MountedFrameDemand(cadence: $0) }
    case .periodic, .explicit:
      schedule.next(after: instant).map { MountedFrameDemand(deadline: $0) }
    }
  }
}
