...
public final class FocusManager {
  ...
  public var activeScopeID: FocusScopeID? {
    scopeStack.last?.scope.id
  }
  ...
  public func register(_ target: FocusTarget) {
    if targets[target.id] == nil {
      registrationOrder.append(target.id)
    }
  ...
  public func moveFocus(forward: Bool = true) -> FocusID? {
    ...
    guard let focusedID, let index = eligible.firstIndex(where: { $0.id == focusedID }) else {
      focusedID = forward ? eligible.first?.id : eligible.last?.id
      return focusedID
    }
  ...
  public func activateScope(_ scope: FocusScope, initialFocus: FocusID? = nil) {
    ...
    if let initialFocus, requestFocus(initialFocus) {
      return
    }
    focusedID = eligibleTargets().first?.id
  }
  ...
  public func deactivateScope(_ id: FocusScopeID) -> Bool {
    ...
    if let restoreID = removed.restoreID, requestFocus(restoreID) {
      return true
    }
    focusedID = eligibleTargets().first?.id
    return true
