...
public struct DiffViewModel: Sendable, Hashable {
  ...
  public init(
    ...
    self.diff = UnifiedDiffParser().parse(source)
...
public struct DiffView: Sendable {
  ...
  public func layout(width: Int) -> DiffLayoutResult {
    ...
    let mode: DiffLayoutMode = switch model.layoutPolicy {
    case .automatic:
      width > Self.sideBySideThreshold ? .sideBySide : .unified
    case .unified:
      .unified
    case .sideBySide:
      .sideBySide
  ...
  private func styled(_ line: DiffLine) -> StyledText {
    let role: SemanticTextRole = switch line.kind {
    case .addition:
      .diffAdded
    case .removal:
      .diffRemoved
