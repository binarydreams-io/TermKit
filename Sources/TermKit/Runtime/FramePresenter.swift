...
public final class FramePresenter {
  ...
  public init(
    ...
    self.resources = ControlRenderResources(
    ...
    self.encoder = ANSIEncoder(colorMode: Self.colorMode(for: session.capabilities.color))
  }
  ...
  public func readInput(maximumByteCount: Int = 4096) throws -> [UInt8] {
  ...
  private func rebuildInternersIfNeeded(
    ...
    let surfaces = [front, back].compactMap(\.self)
    if rebuildGraphemes {
