...
public struct Image: View, SemanticRenderable, Sendable, Hashable {
  ...
  private func renderedCell(
    ...
    return try (glyphs.upper, resources.internPaintStyle(style))
  }
...
private struct Glyphs {
  ...
  init(resources: inout ControlRenderResources) throws {
    self.space = try resources.graphemes.intern(" ")
    self.upper = try resources.graphemes.intern("▀")
    self.lower = try resources.graphemes.intern("▄")
    self.full = try resources.graphemes.intern("█")
  }
}
