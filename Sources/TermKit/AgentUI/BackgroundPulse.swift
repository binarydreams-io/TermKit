...
/// The spatial shape of a background pulse.
public enum BackgroundPulseShape: String, Sendable, Hashable, CaseIterable {
  /// A pulse centered in the available area.
  case radial
  /// A pulse centered near a leading logo.
  case logoAdjacent
}

/// Timing and appearance settings for a background pulse.
public struct BackgroundPulseConfiguration: Sendable, Hashable {
  ...
  /// The number of frames in one pulse cycle.
  public var frameCount: Int
  /// A Boolean value that indicates whether reduced motion is enabled.
  public var isReducedMotionEnabled: Bool

  /// Creates a background pulse configuration.
  public init(
    color: RGBA,
  ...
  /// A Boolean value that indicates whether the pulse requires animated frames.
  /// - Complexity: O(1).
  public var requiresAnimationFrames: Bool {
    isReducedMotionEnabled == false && frameCount > 1
  }

  /// The duration between animation frames.
  /// - Complexity: O(1).
  public var frameInterval: TimeSpan {
    .seconds(1 / Double(framesPerSecond))
  }

  /// Returns the frame index for the elapsed duration.
  /// - Complexity: O(1).
  public func frameIndex(at elapsed: TimeSpan) -> Int {
...
/// A cache key that identifies a generated pulse frame.
public struct BackgroundPulseFrameKey: Sendable, Hashable {
  /// The frame width in cells.
  public var width: Int
  /// The frame height in cells.
  public var height: Int
  /// The frame index within the animation cycle.
  ...
  /// Creates a background pulse frame key.
  public init(
    width: Int,
    height: Int,
    frameIndex: Int,
...
public struct BackgroundPulseCell: Sendable, Hashable {
  ...
  /// Creates a background pulse cell.
...
/// A generated background pulse frame.
public struct BackgroundPulseFrame: Sendable, Hashable {
  /// The frame size in cells.
  public var size: CellSize
  /// The visible cells in the frame.
  public var cells: [BackgroundPulseCell]

  /// Creates a background pulse frame.
...
public struct BackgroundPulse<Frame: Sendable>: Sendable {
  ...
  /// The frame cache used by the pulse.
  ...
  /// Creates a background pulse with an optional initial cache.
  public init(
    configuration: BackgroundPulseConfiguration,
    frameCache: BackgroundPulseFrameCache<Frame> = BackgroundPulseFrameCache()
  ) {
    self.configuration = configuration
    self.cacheStore = BackgroundPulseCacheStore(cache: frameCache)
  }
}

extension BackgroundPulse where Frame == BackgroundPulseFrame {
  /// Creates a pulse that generates standard background pulse frames.
  ...
  func primitive(at instant: TimeInstant, reduceMotion: Bool = false) -> BackgroundPulseCanvas {
    let elapsed = TimeSpan.nanoseconds(max(0, instant.nanoseconds))
...
extension BackgroundPulse: View where Frame == BackgroundPulseFrame {
  /// The view graph for the animated or static pulse.
  ...
  public var graphBody: [NodeDescriptor] {
    if configuration.isReducedMotionEnabled || configuration.requiresAnimationFrames == false {
      ...
      TimelineView(.animation(minimumInterval: configuration.frameInterval)) { context in
        primitive(
          at: context.instant,
          reduceMotion: context.isReducedMotionEnabled || context.areAnimationsEnabled == false
        )
      }.graphBody
    }
  }
...
struct BackgroundPulseCanvas: SemanticRenderable, View, Sendable {
  ...
  nonisolated func paint(
    ...
    for cell in frame.cells {
      ...
      _ = try surface.write(graphemeID: space, at: point, styleID: style, clip: context.clip)
    }
    return SemanticNode(
      id: "background-pulse",
