...
public struct PermissionPrompt: Sendable, Hashable {
  ...
  public var requiresExplicitAction: Bool {
    true
  }

  /// The currently focused permission choice.
  /// - Complexity: O(1).
  public var focusedChoice: PermissionChoice {
...
public struct QuestionPrompt: Sendable, Hashable {
  ...
  private func isCompatible(_ answer: QuestionAnswer?, with question: Question) -> Bool {
    ...
    case let (.singleSelection, .optionIDs(ids)):
      ids.count <= 1 && ids.isSubset(of: Set(question.options.map(\.id)))
    case let (.multipleSelection, .optionIDs(ids)):
  ...
  private func validationError(for rule: QuestionValidationRule, answer: QuestionAnswer?) -> QuestionValidationError? {
    ...
    case .required:
      ...
      case let .optionIDs(ids): ids.isEmpty ? .required : nil
      case let .text(text): text.allSatisfy(\.isWhitespace) ? .required : nil
      case nil: .required
      }
    case let .minimumSelections(minimum):
      guard case let .optionIDs(ids) = answer else { return .tooFewSelections(minimum: minimum) }
      return ids.count < minimum ? .tooFewSelections(minimum: minimum) : nil
    case let .maximumSelections(maximum):
      guard case let .optionIDs(ids) = answer else { return nil }
      return ids.count > maximum ? .tooManySelections(maximum: maximum) : nil
    case let .minimumTextLength(minimum):
      guard case let .text(text) = answer else { return .textTooShort(minimum: minimum) }
      return text.count < minimum ? .textTooShort(minimum: minimum) : nil
    case let .maximumTextLength(maximum):
      guard case let .text(text) = answer else { return nil }
