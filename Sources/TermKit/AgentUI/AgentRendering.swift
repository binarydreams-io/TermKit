...
public struct AgentComponentView<Component: AgentComponentRenderable>: SemanticRenderable, View, Sendable {
  ...
  public nonisolated init(
    ...
    self.isReducedMotionEnabled = reduceMotion
    self.input = nil
    self.preservesExecutableActions = false
    self.timelineCadence = nil
  }
  ...
  init(executable component: Component, theme: ResolvedSemanticTheme) {
    ...
    self.preservesExecutableActions = true
  }
  ...
  private init(
    ...
    self.init(component, theme: theme)
    self.input = AgentComponentInput(
  ...
  private nonisolated func executableSemantics(_ semantics: SemanticNode) -> SemanticNode {
  ...
  ControlFocusTrapping, ControlSemanticActionHandler
{
  ...
  public var trapsControlFocus: Bool {
    input?.trapsFocus == true
  }

  /// Activates the component.
  /// - Complexity: O(1), excluding action work.
  @MainActor public func activate() -> Bool {
    input?.activate() ?? false
  }

  /// Activates the component at a cell point.
  /// - Complexity: O(1), excluding action work.
  @MainActor public func activate(at point: CellPoint) -> Bool {
    input?.activateAt(point) ?? false
  }

  /// Handles a control input event.
  /// - Complexity: O(1), excluding handler work.
  @MainActor public func handleControlInput(_ event: ControlInputEvent) -> Bool {
  ...
  @MainActor public func handleSemanticAction(_ action: SemanticAction) -> Bool {
    if input?.semanticAction(action) == true {
      return true
    }
...
extension AgentComponentView {
  ...
  public init<Body>(
    ...
    self.timelineCadence = component.phase == .running ? .milliseconds(120) : nil
  }
  ...
  public init<Item>(
    ...
    self.init(
      ...
      handle: { event in
        switch event {
        case .moveDown: Self.scrollViewport(state, forward: true, actions: actions)
        case .moveUp: Self.scrollViewport(state, forward: false, actions: actions)
        default: false
        }
      },
      semanticAction: { action in
        switch action {
        case .scrollForward: Self.scrollViewport(state, forward: true, actions: actions)
        case .scrollBackward: Self.scrollViewport(state, forward: false, actions: actions)
        default: false
        }
      }
    )
  ...
  private static func scrollViewport(
    ...
    if forward {
      actions.scrollForward()
    } else {
      actions.scrollBackward()
    }
    return true
  }
  ...
  public init(
    ...
    let select: @MainActor @Sendable () -> Bool = {
      ...
      if case let .optionIDs(ids) = value.answers[question.id] {
        selected = ids
      }
    ...
    let advance: @MainActor @Sendable () -> Bool = {
      ...
      if viewState.moveToNextStep(in: value) {
        if value.questions[viewState.stepIndex].kind != .customText {
          viewState.focusedOptionIndex = 0
        }
        state.wrappedValue = viewState
        return true
      }
    ...
    self.init(
      ...
      handle: { event in
        ...
        case .moveUp where question.kind != .customText,
          ...
          state.wrappedValue = viewState
          return true
        case let .text(text) where question.kind == .customText,
             .paste(let text) where question.kind == .customText:
          let current: String = if case let .text(answer) = value.answers[question.id] {
            answer
          } else {
            ""
          }
        ...
        case .deleteBackward where question.kind == .customText:
          guard case var .text(text) = value.answers[question.id], text.isEmpty == false else { return true }
          text.removeLast()
...
extension String: AgentContentPresentable {
  ...
  public var agentStyledText: StyledText {
    StyledText(self)
  }
}
...
private enum AgentGrid {
  ...
  static func duration(_ duration: TimeSpan) -> String {
    if duration.seconds < 1 {
      return "\(duration.nanoseconds / 1000000)ms"
    }
...
extension AgentPrompt: AgentComponentRenderable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if configuration.isBusy {
      lines.append(StyledText("Working...", role: .muted))
    }
...
extension AgentPromptRenderLeaf: SemanticRenderable {
  ...
  private nonisolated var resolvedTheme: ResolvedSemanticTheme {
    do {
...
extension UserMessageCard: AgentComponentRenderable where Attachment: AgentContentPresentable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if isQueued {
      footer += footer.isEmpty ? "Queued" : " · Queued"
    }
    if footer.isEmpty == false {
      lines.append(StyledText("   " + footer, role: .muted))
    }
...
extension ReasoningDisclosure: AgentComponentRenderable where Body: AgentContentPresentable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    let marker: String = if phase == .running {
      context.isReducedMotionEnabled ? "..." : ["·", "●", "•"][Int(max(0, context.elapsed.nanoseconds) / 120_000_000) % 3]
    } else {
      isExpanded ? "▾" : "▸"
    }
    var header = "\(marker) \(label)"
    if phase == .completed, let summary, summary.isEmpty == false {
      header += " · \(summary)"
    }
    if phase == .completed, let duration {
      header += " · \(AgentGrid.duration(duration))"
    }
    var lines = [StyledText(header, role: .muted)]
    if isExpanded {
      lines.append(body.agentStyledText)
    }
...
extension ToolCallRow: AgentComponentRenderable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if revealsError, let errorBody {
      lines.append(StyledText(errorBody, role: .diagnostic))
    }
    var semanticState: SemanticState = []
    if state == .running {
      semanticState.insert(.busy)
    }
    if revealsError {
      semanticState.insert(.expanded)
    }
...
extension ShellResult: AgentComponentRenderable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if let workingDirectory {
      lines.append(StyledText(workingDirectory, role: .muted))
    }
    ...
    if let limit {
      lines.append(StyledText("… \(outputLines.count - limit) more lines", role: .muted))
    }
    if isRunning {
      lines.append(StyledText(context.isReducedMotionEnabled ? "Running..." : "● Running", role: .muted))
    }
    if hasFailed {
      lines.append(StyledText("Exit \(exitCode ?? 1)", role: .diagnostic))
    }
    var state: SemanticState = []
    if isRunning {
      state.insert(.busy)
  ...
  ToolActivity: AgentContentPresentable, Diagnostic: AgentContentPresentable
{
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if let footer {
      var fields = [footer.agentMode, footer.model].compactMap(\.self)
      if let duration = footer.duration {
        fields.append(AgentGrid.duration(duration))
      }
      if footer.wasInterrupted {
        fields.append("Interrupted")
      }
      if fields.isEmpty == false {
        lines.append(StyledText(fields.joined(separator: " · "), role: .muted))
      }
    }
    let children = [
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    for (questionIndex, question) in questions.enumerated() {
      ...
      if let detail = question.detail {
        lines.append(StyledText(detail, role: .muted))
      }
      switch question.kind {
      case .customText:
        let value: String = if case let .text(text) = answers[question.id] {
          text
        } else {
          "Enter a response"
        }
      ...
      case .singleSelection, .multipleSelection:
        let selected: Set<String> = if case let .optionIDs(ids) = answers[question.id] {
          ids
        } else {
          []
        }
...
extension TodoItem: AgentComponentRenderable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    if state == .completed {
      semanticState.insert(.checked)
    }
    if state == .cancelled {
      semanticState.insert(.disabled)
    }
    return AgentGrid.output(
      id: "todo-item",
      role: .listItem,
...
extension SessionSidebar: AgentComponentRenderable where Item: AgentContentPresentable {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    var state: SemanticState = []
    if policy.placement(forTerminalWidth: context.width) == .trailingOverlay {
      state.insert(.modal)
    }
    return AgentGrid.output(
      id: "session-sidebar",
...
extension BackgroundPulse: AgentComponentRenderable where Frame == BackgroundPulseFrame {
  ...
  public func render(in context: AgentRenderContext) -> AgentRenderOutput {
    ...
    var state: SemanticState = []
    if configuration.requiresAnimationFrames, context.isReducedMotionEnabled == false {
      state.insert(.busy)
    }
    return AgentGrid.output(
      id: "background-pulse",
