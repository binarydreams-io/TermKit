...
public struct CellDiffer: Sendable, Hashable {
  ...
  private func mergedRuns(from marks: [Bool]) -> [Range<Int>] {
    ...
    while index < marks.count {
      ...
      let start = index
      while index < marks.count, marks[index] {
        index += 1
      }
...
public struct SemanticScreen: Sendable, Equatable {
  ...
  public mutating func apply(_ operations: some Sequence<SemanticOperation>) throws {
    for operation in operations {
      ...
      case let .setStyle(identifier):
        styleID = identifier
      case let .write(graphemeID, displayWidth, flags):
        try surface.write(
