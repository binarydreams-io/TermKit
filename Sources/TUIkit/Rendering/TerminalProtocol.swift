...
public protocol TerminalProtocol: AnyObject, Sendable {
  ...
  /// Reads a keyboard or mouse event from the terminal.
  ///
  /// - Returns: The input event, or `nil` if no input is available.
  ...
  ///
  /// The title is written immediately, outside any active frame buffer.
  /// The runtime saves the previous title at startup and restores it on
  /// exit, so a title set here never outlives the application.
  ///
  ...
  ///
  /// While reporting is off, the terminal handles drag gestures itself,
  /// which lets the user select and copy text. Reporting belongs to the
  /// raw session, so this does nothing before raw mode is enabled.
  ///
...
extension TerminalProtocol {
  /// Reads a keyboard event from conforming types that only implement key input.
  public func readInputEvent() -> InputEvent? {
    readKeyEvent().map(InputEvent.key)
  }
}

/// Exposes buffered input state to the application event loop.
@MainActor
protocol TerminalInputScheduling: AnyObject {
  /// Whether the terminal can return input without another descriptor wake-up.
  var hasBufferedInput: Bool { get }

  /// The delay before the runtime retries an ambiguous input prefix.
  var pendingInputRetryDelay: Duration? { get }
}

// MARK: - Terminal I/O Failure Reporting

/// A terminal system call that failed permanently.
struct TerminalIOFailure: Error, Equatable, Sendable, CustomStringConvertible {
...
@MainActor
protocol TerminalFailureReporting: AnyObject {
