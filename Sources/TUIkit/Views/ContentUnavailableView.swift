...
private struct _ContentUnavailableViewCore<Label: View, Description: View, Actions: View>: View, Renderable {
  ...
  func renderToBuffer(context: RenderContext) -> FrameBuffer {
    ...
    let labelBuffer = TUIkit.renderToBuffer(
      label,
      context: context.withIdentityScope("contentUnavailable.label")
    )

    // Render description with secondary foreground color
    var descContext = context.withIdentityScope("contentUnavailable.description")
    ...
    let actionsBuffer = TUIkit.renderToBuffer(
      actions,
      context: context.withIdentityScope("contentUnavailable.actions")
    )
    ...
    var horizontalOffsets: [Int] = []
    centeredLines.reserveCapacity(result.lines.count)
    horizontalOffsets.reserveCapacity(result.lines.count)
    ...
    for line in result.lines {
      ...
      horizontalOffsets.append(leftPad)
    }

    let centeredRegions = result.regions.flatMap { region in
      (region.rect.y ..< region.rect.maxY).map { row in
        InteractionRegion(
          id: region.id,
          rect: TerminalCellRect(
            x: region.rect.x + horizontalOffsets[row],
            y: row,
    ...
    return FrameBuffer(lines: centeredLines, regions: centeredRegions)
  }
}
