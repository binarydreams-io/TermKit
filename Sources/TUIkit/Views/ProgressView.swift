...
private struct _ProgressViewCore<Label: View, CurrentValueLabel: View>: View, Renderable {
  ...
  private func renderLabelLine(width: Int, palette: any Palette, context: RenderContext) -> String {
    let labelBuffer: FrameBuffer = if let labelView = label, !(labelView is EmptyView) {
      TUIkit.renderToBuffer(labelView, context: context)
    } else {
      FrameBuffer()
    }

    let valueBuffer: FrameBuffer = if let valueView = currentValueLabel, !(valueView is EmptyView) {
      TUIkit.renderToBuffer(valueView, context: context)
    } else {
      FrameBuffer()
    }
