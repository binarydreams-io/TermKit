...
extension PreferenceModifier: Renderable {
    func renderToBuffer(context: RenderContext) -> FrameBuffer {
        // Set the preference value. Skipped during measurement passes: a
        // stack's two-pass layout renders each non-Layoutable child once to
        // measure it and once for real (see `ChildInfo.measureChild`'s
        // render-to-measure fallback), and this side effect must run only
        // on the real pass or every preference gets set twice.
