...
internal final class RuntimeEventChannel: Sendable {
    ...
    /// Whether an unread input-readiness event is already queued.
    private let hasPendingInput = Lock(initialState: false)

    /// Whether an unread render request is already queued.
    private let hasPendingRender = Lock(initialState: false)
    ...
    func send(_ event: RuntimeEvent) {
        ...
        switch event {
        case .inputAvailable:
            shouldSend = hasPendingInput.withLock { isPending in
                guard !isPending else { return false }
                isPending = true
                return true
            }
        case .renderRequested:
            shouldSend = hasPendingRender.withLock { isPending in
                guard !isPending else { return false }
                isPending = true
                return true
            }
    ...
    /// Marks one event as removed from the runtime queue.
    func didConsume(_ event: RuntimeEvent) {
        switch event {
        case .inputAvailable:
            hasPendingInput.withLock { isPending in
                isPending = false
            }
        case .renderRequested:
            hasPendingRender.withLock { isPending in
                isPending = false
            }
        default:
            break
