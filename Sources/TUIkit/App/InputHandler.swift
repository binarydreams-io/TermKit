...
internal struct InputHandler {
    ...
    /// The dispatcher for mouse interactions in the active frame.
...
extension InputHandler {
    /// Dispatches a keyboard or mouse input event.
    ///
    /// Clicks that represent keys return to `handle(_:)` so all
    /// keyboard guards and priorities remain active.
    func handle(_ event: InputEvent) {
        switch event {
        case .key(let keyEvent):
            handle(keyEvent)
        case .mouse(let mouseEvent):
            if let keyEvent = interactionDispatcher.dispatch(mouseEvent) {
                handle(keyEvent)
            }
        }
    }
