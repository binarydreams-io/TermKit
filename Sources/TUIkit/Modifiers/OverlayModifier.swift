...
extension OverlayModifier: Renderable {
  public func renderToBuffer(context: RenderContext) -> FrameBuffer {
    ...
    let baseBuffer = TUIkit.renderToBuffer(
      base,
      context: context.withIdentityScope("overlay.base")
    )
    let overlayBuffer = TUIkit.renderToBuffer(
      overlay,
      context: context.withIdentityScope("overlay.overlay")
    )
    ...
    let horizontalOffset: Int = switch alignment.horizontal {
    case .leading:
      0
    case .center:
      max(0, (baseWidth - overlayWidth) / 2)
    case .trailing:
      max(0, baseWidth - overlayWidth)
    }
    ...
    case .top:
      0
    case .center:
      max(0, (baseHeight - overlayHeight) / 2)
    case .bottom:
      max(0, baseHeight - overlayHeight)
    }
