...
extension ModalPresentationModifier: Renderable {
    ...
    public func renderToBuffer(context: RenderContext) -> FrameBuffer {
        ...
        // The base content uses throwaway focus, key, and mouse dispatchers.
        // Its interactive elements cannot interfere with the modal.
        let dimmedBase = DimmedModifier(content: content)
        let isolatedContext = context.isolatedForBackground()
        var dimmedBuffer = TUIkit.renderToBuffer(dimmedBase, context: isolatedContext)
        dimmedBuffer.regions.removeAll()
        ...
        interactionDispatcher?.beginExclusiveScope()
        defer { interactionDispatcher?.endExclusiveScope() }
