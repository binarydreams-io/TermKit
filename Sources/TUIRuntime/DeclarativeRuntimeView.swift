...
@MainActor
final class DeclarativeRuntimeView<Root: TUIViewGraph.View>: IncrementalRuntimeView, DeclarativeSemanticActionRuntimeView {
    ...
    private var retainedSurface: TUIRenderer.Surface?
    ...
    private struct IncrementalSnapshot {
        ...
        let retainedSurface: TUIRenderer.Surface?
    ...
    func nodeDescriptor(in context: RuntimeFrameContext) -> NodeDescriptor {
        ...
        return NodeDescriptor.declarative(root).scopedExpansion { _, body in
    ...
    func updatePresentation(in context: RuntimeFrameContext, graph: ViewGraph, layout: Bool) throws {
        if layout {
            ...
            if let root = graph.root { try samplePresentation(in: root, context: context) }
            for root in graph.presentationRoots { try samplePresentation(in: root, context: context) }
        } else {
    ...
    private func sampleAndLayout(in context: RuntimeFrameContext, graph: ViewGraph) throws {
        ...
        if let root = graph.root {
            ...
            if usesReducedMotion { applyReducedTargets(in: root) }
    ...
    func paint(
        ...
        let canIncrement = retainedSurface?.size == context.terminalSize
            && hasStructuralInvalidation(in: graph?.root) == false
            && externalDamage?.isEmpty != false
            && requiresFullPaint == false
        var surface = canIncrement ? retainedSurface ?? TUIRenderer.Surface(size: context.terminalSize)
            : TUIRenderer.Surface(size: context.terminalSize)
        ...
        if canIncrement {
            ...
            for rect in paintDamage.rectangles { surface.clear(rect) }
        } else {
        ...
        if canIncrement {
            var expanded = DamageTracker(bounds: frameBounds)
            for rect in paintDamage.rectangles { expanded.add(surface.atomExpanded(rect)) }
            paintDamage = expanded
        }
        ...
        for rect in previousAnimatedBounds.values { damage.add(rect) }
    ...
    func dispatch(_ event: TerminalInputEvent) {
        ...
        switch event {
        case .key(let key) where key.action != .release && key.key == .tab:
        ...
        case .key(let key) where key.action != .release:
        ...
        case .mouse(let mouse):
            ...
            if node.isFocusable { setFocusedNodeID(node.id, in: graph) }
    ...
    private func semanticFrame(for id: SemanticID) -> CellRect? {
        retainedSemantics.values.lazy.flatMap { $0 }.compactMap { $0.node(withID: id)?.frame }.first
    }
    ...
    private func dispatchShortcut(_ key: TerminalKeyEvent, to node: MountedNode) -> Bool {
        ...
        if key.modifiers.contains(.alt) { modifiers.insert(.option) }
        if key.modifiers.contains(.shift) { modifiers.insert(.shift) }
        if key.modifiers.contains(.super) { modifiers.insert(.command) }
        let handled = handler.handleKeyboardShortcut(KeyboardShortcut(mappedKey, modifiers: modifiers))
        if handled { node.invalidate(.paint) }
        return handled
    }
    ...
    private func samplePresentation(in node: MountedNode, context: RuntimeFrameContext) throws {
        try samplePresentationValue(in: node, context: context)
        for child in node.children { try samplePresentation(in: child, context: context) }
    }
    ...
    private func samplePresentationValue(in node: MountedNode, context: RuntimeFrameContext) throws {
        func value<Value: VectorArithmetic>(_ property: PresentationProperty<Value>) throws -> Value? {
            ...
            guard presentationValueIsFinite(value) else {
                throw TUIRuntimeError.invalidPresentationValue(
        ...
        func running<Value: VectorArithmetic>(_ property: PresentationProperty<Value>) -> Bool {
        ...
        presentation.isAnimating = running(.foregroundColor) || running(.backgroundColor)
    ...
    private func forwardLayoutPresentation(in node: MountedNode?) {
        ...
        for child in node.children { forwardLayoutPresentation(in: child) }
    ...
    private func applyReducedTargets(in node: MountedNode) {
        ...
        presentations[node.id] = presentation
        for child in node.children { applyReducedTargets(in: child) }
    }

    private func firstLayoutPrimitive(in node: MountedNode?) -> MountedNode? {
        guard let node else { return nil }
        if node.primitive(as: LayoutPrimitive.self) != nil { return node }
    ...
    private func layoutResult(for node: MountedNode, proposal: ProposedCellSize) -> LayoutResult {
        ...
        guard let primitive = node.primitive(as: LayoutPrimitive.self) else {
            ...
            return LayoutResult(size: natural, placements: zip(items, sizes).map {
                LayoutPlacement(nodeID: $0.nodeID, frame: CellRect(origin: .zero, size: $1))
            })
        }
        let presentation = presentations[node.id] ?? Presentation()
        switch primitive {
        case .stack(var layout):
            if let spacing = presentation.spacing { layout.spacing = nonnegative(spacing) }
            return layout.layout(items, in: proposal, cache: layoutCache)
        case .padding(var layout):
            guard let item = items.first else { return LayoutResult(size: .zero, placements: []) }
            if let padding = presentation.padding { layout.insets = edgeInsets(padding) }
            return layout.layout(item, in: proposal, cache: layoutCache)
        case .frame(var layout):
            guard let item = items.first else { return LayoutResult(size: .zero, placements: []) }
            if let width = presentation.frameWidth { layout.width = nonnegative(width) }
            if let height = presentation.frameHeight { layout.height = nonnegative(height) }
            return layout.layout(item, in: proposal, cache: layoutCache)
        case .scrollViewport(let layout):
            guard let item = items.first else { return LayoutResult(size: .zero, placements: []) }
    ...
    private func place(
        ...
        let presentation = presentations[node.id] ?? Presentation()
        let transitionOffset = presentation.transition?.offset ?? .zero
        let offsetValue = presentation.offset
    ...
    private func paintNode(
        _ node: MountedNode,
        into destination: inout TUIRenderer.Surface,
        damage: DamageTracker?,
        resources: inout ControlRenderResources
    ) throws -> [SemanticNode] {
        if let damage, contributes(node, to: damage) == false { return retainedSemantics[node.id] ?? [] }
        ...
        let selectionHighlight = presentation.selectionHighlight
            ?? node.presentationTarget(for: PresentationProperties.selectionHighlight)
        let isolates = opacity < 1 || presentation.transition?.symbol != nil
    ...
    private func paintSubtree(
        ...
        if let renderable = node.primitive(as: (any SemanticRenderable).self), let frame = node.cachedFrame {
            ...
            if node.id == focusedNodeID { semantic.state.insert(.focused) }
            semantics.append(semantic)
        }
        let children = paintContributorIDs == nil || rebuildSemanticAncestors
        ...
        let highlight = state.contains(.focused) ? focusHighlight
        ...
        if let symbol = presentation.transition?.symbol, let character = symbol.first,
           let point = firstVisibleCell(in: node.paintBounds, on: surface) {
            ...
            _ = try surface.write(graphemeID: grapheme, styleID: cell.styleID, at: point, clip: clips[node.id])
        }
        return semantics
    }

    private func contributes(_ node: MountedNode, to damage: DamageTracker) -> Bool {
        if let paintContributorIDs { return paintContributorIDs.contains(node.id) }
        if damage.rectangles.contains(where: node.paintBounds.intersects) { return true }
    ...
    private func sceneChildren(of parent: MountedNode) -> [MountedNode] {
        if let cached = sceneChildCache[parent.id] { return cached }
    ...
    private func rebuildSceneChildCache(graph: ViewGraph) {
        ...
        func cache(_ node: MountedNode) {
            ...
            for child in children { cache(child) }
        }
        if let root = graph.root { cache(root) }
        for root in scenePresentationRoots(parentID: nil) { cache(root) }
    }
    ...
    private func sceneChildrenUncached(of parent: MountedNode) -> [MountedNode] {
        ...
        for node in retained {
            ...
            let index: Int
            if let nextID = placement?.nextSiblingID,
               let nextIndex = siblings.firstIndex(where: { $0.id == nextID }) {
                ...
                index = min(placement?.siblingIndex ?? siblings.count, siblings.count)
            }
    ...
    private func firstVisibleCell(in bounds: CellRect, on surface: TUIRenderer.Surface) -> CellPoint? {
        guard let bounds = surface.bounds.intersection(bounds) else { return nil }
        for y in bounds.minY..<bounds.maxY {
            for x in bounds.minX..<bounds.maxX {
                let point = CellPoint(x: x, y: y)
                let cell = surface[point]
                if cell.isTransparent == false && cell.isContinuation == false { return point }
            }
        }
        return nil
    }
    ...
    private func drawBorder(
        ...
        for y in frame.minY..<frame.maxY {
            for x in frame.minX..<frame.maxX where y == frame.minY || y == frame.maxY - 1 || x == frame.minX || x == frame.maxX - 1 {
                ...
                _ = try surface.write(graphemeID: grapheme, styleID: style, at: point, clip: clip)
            }
        }
    }
    ...
    private func resolveRuntimeColor(_ color: Color?, role: CellColorRole) throws -> RGBA {
        ...
        case .semantic(let semantic): throw ColorResolutionError.missing(semantic)
    ...
    private func nonnegative(_ value: Double) -> Int { max(0, quantize(value)) }
    ...
    private func presentedHitTest(
        ...
        for child in sceneChildren(of: node).reversed() {
            if let match = presentedHitTest(point, in: child, requiredModalScope: requiredModalScope) { return match }
        }
        guard node.acceptsHitTesting else { return nil }
        return requiredModalScope == nil || node.activeModalScope == requiredModalScope ? node : nil
    }
    ...
    private func moveFocus(backward: Bool, in graph: ViewGraph) {
        ...
        guard nodes.isEmpty == false else { setFocusedNodeID(nil, in: graph); return }
