...
public struct RichText: SemanticRenderable, TUIViewGraph.View, Sendable, Hashable {
    ...
    nonisolated public func sizeThatFits(_ proposal: ProposedCellSize) -> CellSize {
        let naturalWidth = text.plainText
...
public enum SemanticCellPainter {
    ...
    private static func style(for cell: SemanticCell, theme: ResolvedSemanticTheme) -> CellStyle {
        ...
        if cell.attributes.contains(.bold) { attributes.insert(.bold) }
        if cell.attributes.contains(.italic) { attributes.insert(.italic) }
        if cell.attributes.contains(.underline) { attributes.insert(.underline) }
        if cell.attributes.contains(.strikethrough) { attributes.insert(.strikethrough) }
        if cell.attributes.contains(.dim) { attributes.insert(.dim) }
        return CellStyle(
...
public struct RichTextRenderResult: Sendable {
    public let surface: TUIRenderer.Surface
...
public struct MarkdownPresentation: Sendable {
    ...
    public func render(
        ...
        for (index, block) in document.blocks.enumerated() {
            if index > 0 { rows.append(Array(repeating: .blank(), count: width)) }
        ...
        if rows.isEmpty { rows = [Array(repeating: .blank(), count: width)] }
        return try RichTextPacker(theme: theme, scheme: scheme).pack(
            rows: rows,
            id: "markdown",
    ...
    private func tableRows(_ table: MarkdownTable, width: Int) -> [[CellSpec]] {
        ...
        return allRows.map { values in
            ...
            for column in 0..<columns {
                if column > 0 { text = text.appending(StyledText(" │ ", role: .muted)) }
...
public struct CodeBlock: Sendable {
    ...
    public func render(
        ...
        var rows: [[CellSpec]] = []
        if let title = layout.title { rows.append(makeRow(title, width: width, background: .element)) }
...
extension DiffView {
    public func render(
        ...
        for layoutRow in result.rows {
            switch layoutRow {
            case .fileHeader(let oldPath, let newPath):
                let label = [oldPath, newPath].compactMap { $0 }.joined(separator: " → ")
            ...
            case .unified(let old, let new, let content, let kind):
                ...
                if width > 9 { cells += specs(content, width: width - 9, background: background(for: kind)) }
                rows.append(Array(cells.prefix(width)))
            case .sideBySide(let left, let right):
    ...
    private func background(for kind: DiffLineKind) -> SemanticColorRole {
        switch kind {
        case .addition: return .diffAdditionBackground
        case .removal: return .diffDeletionBackground
        case .context, .noNewlineMarker: return .panel
        }
    }
}
...
private struct RichTextPacker {
    ...
    func pack(
        ...
        var surface = TUIRenderer.Surface(size: CellSize(width: width, height: rows.count))
    ...
    private func foregroundRole(for role: SemanticTextRole) -> SemanticColorRole {
        ...
        case .inlineCode, .code: return .markdownCode
        ...
        case .diffAdded: return .diffAddition
        case .diffRemoved: return .diffDeletion
        case .diffContext: return .diffContext
        case .diffHunk: return .accent
        case .muted, .lineNumber, .listMarker, .quoteMarker: return .mutedText
        case .diagnostic: return .error
        case .body, .tableHeader: return .text
        }
    }

    private func backgroundRole(for role: SemanticTextRole) -> SemanticColorRole {
        switch role {
        case .inlineCode, .code, .keyword, .string, .number, .comment, .type: return .panel
        case .diffAdded: return .diffAdditionBackground
        case .diffRemoved: return .diffDeletionBackground
    ...
    private func textAttributes(_ attributes: StyledTextAttributes) -> TextAttributes {
        ...
        if attributes.contains(.bold) { result.insert(.bold) }
        if attributes.contains(.italic) { result.insert(.italic) }
        if attributes.contains(.underline) { result.insert(.underline) }
        if attributes.contains(.strikethrough) { result.insert(.strikethrough) }
        if attributes.contains(.dim) { result.insert(.dim) }
        return result
    }
}
