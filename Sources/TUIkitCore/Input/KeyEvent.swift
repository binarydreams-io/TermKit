...
private enum ASCIIByte {
  // Control characters
  static let backspace: UInt8 = 0x08, tab: UInt8 = 0x09
  static let lineFeed: UInt8 = 0x0A, carriageReturn: UInt8 = 0x0D
  static let escape: UInt8 = 0x1B, delete: UInt8 = 0x7F
  // Ctrl+key range (Ctrl+A = 0x01 … Ctrl+Z = 0x1A)
  static let ctrlRangeStart: UInt8 = 0x01, ctrlRangeEnd: UInt8 = 0x1A
  static let ctrlToLowerOffset: UInt8 = 0x60
  // Printable ASCII range
  static let printableStart: UInt8 = 0x20, printableEnd: UInt8 = 0x7E
  // CSI introducer
...
extension KeyEvent {
  ...
  private static func parseSingleByte(_ byte: UInt8) -> KeyEvent? {
    switch byte {
    case ASCIIByte.escape: return KeyEvent(key: .escape)
    case ASCIIByte.carriageReturn, ASCIIByte.lineFeed: return KeyEvent(key: .enter)
    case ASCIIByte.tab: return KeyEvent(key: .tab)
    case ASCIIByte.delete, ASCIIByte.backspace: return KeyEvent(key: .backspace)
    case 0x20: return KeyEvent(key: .space) // Space
  ...
  private static func parseEscapeSequence(_ bytes: [UInt8]) -> KeyEvent? {
    ...
    if bytes.count == 2, let keyEvent = parseSingleByte(bytes[1]) {
  ...
  private static func parseSS3Sequence(_ byte: UInt8) -> KeyEvent? {
    switch byte {
    case 0x50: KeyEvent(key: .f1) // 'P'
    case 0x51: KeyEvent(key: .f2) // 'Q'
    case 0x52: KeyEvent(key: .f3) // 'R'
    case 0x53: KeyEvent(key: .f4) // 'S'
    default: nil
    }
  }
  ...
  private static func parseCSISequence(_ params: [UInt8]) -> KeyEvent? {
    ...
    switch params.last {
    case ASCIIByte.arrowUp: return KeyEvent(key: .up, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.arrowDown: return KeyEvent(key: .down, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.arrowRight: return KeyEvent(key: .right, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.arrowLeft: return KeyEvent(key: .left, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.home: return KeyEvent(key: .home, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.end: return KeyEvent(key: .end, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
    case ASCIIByte.tilde: return parseExtendedKey(params, modifiers: modifiers)
  ...
  private static func parseExtendedKey(
    ...
    let numberBytes: ArraySlice<UInt8> = if let semicolonIndex = params.firstIndex(of: 0x3B) {
      params[..<semicolonIndex]
    } else {
      params.dropLast()
    }
    ...
    return KeyEvent(key: key, ctrl: modifiers.ctrl, alt: modifiers.alt, shift: modifiers.shift)
  }
