...
extension EquatableView: Renderable {
  public func renderToBuffer(context: RenderContext) -> FrameBuffer {
    ...
    // Measurement passes must not populate or use the runtime cache.
    if !context.isMeasuring, let cached = cache.lookup(
      identity: identity,
      view: content,
      contextWidth: context.availableWidth,
      contextHeight: context.availableHeight
    ), cached.regions.isEmpty {
    ...
    // Render cache misses and interactive subtrees normally.
    let buffer = TUIkitView.renderToBuffer(content, context: context)

    if !context.isMeasuring, buffer.regions.isEmpty {
      cache.store(
        identity: identity,
        view: content,
        buffer: buffer,
...
extension EquatableView {
  ...
  private func markSubtreeActive(context: RenderContext) {
