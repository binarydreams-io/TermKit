...
public struct AgentRenderContext: Sendable, Hashable {
    ...
    public var elapsed: TUIDuration
    public var reduceMotion: Bool

    public init(
        width: Int,
        theme: ResolvedSemanticTheme,
        elapsed: TUIDuration = .zero,
...
private extension ColorScheme {
    var snapshotName: String { self == .light ? "light" : "dark" }
}

private extension RGBA {
    var snapshotRGBA: String {
...
private extension SemanticTextRole {
...
private protocol AgentTimelineComponent {
    var requestedTimelineCadence: TUIDuration? { get }
}

public struct AgentComponentView<Component: AgentComponentRenderable>: SemanticRenderable, TUIViewGraph.View, Sendable {
    ...
    private var timelineCadence: TUIDuration?
    ...
    nonisolated public func sizeThatFits(_ proposal: ProposedCellSize) -> CellSize {
        ...
        let output = component.render(
            in: AgentRenderContext(width: width, theme: theme, elapsed: elapsed, reduceMotion: reduceMotion)
        )
        return CellSize(
    ...
    nonisolated public func paint(
        ...
        guard context.clip.width > 0, context.clip.height > 0 else {
            let output = component.render(
                in: AgentRenderContext(width: 1, theme: theme, elapsed: elapsed, reduceMotion: reduceMotion)
            )
    ...
    public var graphBody: [NodeDescriptor] {
        let requestedCadence = timelineCadence
        ...
        if let requestedCadence {
            return TimelineView<AgentComponentPrimitiveView<Component>>(
                ...
                view.reduceMotion = timeline.reduceMotion || timeline.animationsEnabled == false
...
private struct AgentComponentPrimitiveView<Component: AgentComponentRenderable>: TUIViewGraph.View {
...
public extension AgentComponentView {
    @MainActor
    init<ID>(
        ...
        self.init(
            component,
            theme: theme,
            activate: { actions.remove(component.id); return true },
            activateAt: { _ in actions.remove(component.id); return true },
    ...
    init(
        ...
        let select: @MainActor @Sendable () -> Bool = {
            ...
            guard question.kind != .customText,
                  ...
                  question.options.indices.contains(optionIndex) else { return false }
        ...
        let advance: @MainActor @Sendable () -> Bool = {
            ...
            guard viewState.stepIndex == value.questions.count - 1,
                  value.isQuestionValid(at: viewState.stepIndex) else { return false }
...
private enum AgentGrid {
    ...
    static func duration(_ duration: TUIDuration) -> String {
...
extension AgentPrompt: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        let localContext = AgentRenderContext(width: width, theme: context.theme, elapsed: context.elapsed,
                                              reduceMotion: context.reduceMotion)
        var semanticChildren = [SemanticNode(
        ...
        if configuration.isBusy {
            semanticChildren.append(SemanticNode(
...
extension AgentPromptRenderLeaf: SemanticRenderable {
    ...
    nonisolated private var componentView: AgentComponentView<AgentPrompt<Attachment>> {
...
extension ToastPresentation: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        let role: SemanticTextRole = switch kind {
        case .failure: .diagnostic
        case .information, .success, .warning: .body
        }
        let line = AgentGrid.line(
            AgentGrid.span("▌ ", role: kind == .failure ? .diagnostic : .listMarker),
...
extension UserMessageCard: AgentComponentRenderable where Attachment: AgentContentPresentable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        for attachment in attachments {
            lines.append(AgentGrid.line(AgentGrid.span("▌  ", role: .listMarker),
        ...
        var output = AgentGrid.output(id: "user-message", role: .group, label: text, lines: lines,
                                context: context, state: state,
                                background: isHovered ? .element : .panel)
        output.cells = SemanticCellGrid(width: output.cells.width, rows: output.cells.rows.map { row in
            row.map { cell in
...
extension ReasoningDisclosure: AgentComponentRenderable where Body: AgentContentPresentable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        let marker: String
        if phase == .running {
            marker = context.reduceMotion ? "..." : ["·", "●", "•"][Int(max(0, context.elapsed.nanoseconds) / 120_000_000) % 3]
        } else {
            marker = isExpanded ? "▾" : "▸"
        }
        ...
        return AgentGrid.output(id: "reasoning", role: .button, label: label, lines: lines,
                                context: context, state: state, actions: actions)
    }
}

extension ReasoningDisclosure: AgentTimelineComponent {
    fileprivate var requestedTimelineCadence: TUIDuration? {
...
extension ToolCallRow: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        return AgentGrid.output(id: "tool-call", role: .group, label: label, lines: lines,
                                context: context, state: semanticState, actions: actions)
    }
}
...
extension ToolResultPanel: AgentComponentRenderable where Content: AgentContentPresentable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        guard isVisibleAsPanel else {
            return AgentGrid.output(id: "tool-result", role: .group, label: title,
                                    lines: [], context: context)
        }
        let lines = [
            AgentGrid.line(AgentGrid.span("▌ ", role: .listMarker), AgentGrid.span(title, role: .muted)),
            AgentGrid.line(AgentGrid.span("▌ ", role: .listMarker),
                           AgentGrid.span(content.agentStyledText.plainText)),
        ]
        return AgentGrid.output(id: "tool-result", role: .group, label: title, lines: lines,
                                context: context, background: .panel)
    }
}

extension ShellResult: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        var lines = [AgentGrid.line(AgentGrid.span("$ ", role: .listMarker),
        ...
        if isRunning { lines.append(StyledText(context.reduceMotion ? "Running..." : "● Running", role: .muted)) }
        ...
        return AgentGrid.output(id: "shell-result", role: .group, label: command, lines: lines,
                                context: context, state: state, actions: limit == nil ? [] : [.activate])
    }
}
...
extension DiagnosticsList: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        if mode == .inline {
            return AgentGrid.output(id: "diagnostics", role: .status, label: inlineSummary ?? "No diagnostics",
                                    lines: inlineSummary.map { [StyledText($0, role: .diagnostic)] } ?? [], context: context)
        }
        let lines = diagnostics.map { diagnostic in
            let symbol = switch diagnostic.severity {
            case .information: "i"
            case .warning: "!"
            case .error: "×"
            }
        ...
        let children = diagnostics.enumerated().map { index, diagnostic in
            SemanticNode(id: SemanticID(rawValue: "diagnostic-\(index)"), role: .button,
                         label: diagnostic.message, actions: [.activate])
        }
        return AgentGrid.output(id: "diagnostics", role: .list, label: "Diagnostics", lines: lines,
                                context: context, actions: diagnostics.isEmpty ? [] : [.focus], children: children)
    }
}
...
extension ConversationViewport: AgentComponentRenderable where Item: AgentContentPresentable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        let children = visibleRange.map { index in
            SemanticNode(id: SemanticID(rawValue: "conversation-item-\(index)"), role: .group,
                         label: items[index].agentStyledText.plainText)
        }
        return AgentGrid.output(id: "conversation", role: .scrollView, label: "Conversation",
                                lines: lines, context: context, state: semanticState,
                                actions: [.scrollForward, .scrollBackward], children: children)
    }
}

extension AssistantMessage: AgentComponentRenderable
where Markdown: AgentContentPresentable, Reasoning: AgentContentPresentable,
    ...
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        return AgentGrid.output(id: "assistant-message", role: .group, label: "Assistant response",
                                lines: lines, context: context, children: children)
    }
}
...
extension PermissionPrompt: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        var lines = [AgentGrid.line(AgentGrid.span("Permission required", role: riskRole, attributes: .bold)),
                     StyledText(requestedAction)]
        lines.append(contentsOf: resources.map { StyledText("• \($0)", role: .muted) })
        lines.append(contentsOf: choices.enumerated().map { index, choice in
            AgentGrid.line(
                AgentGrid.span(index == focusedChoiceIndex ? "▌ " : "  ", role: .listMarker),
                AgentGrid.span(choice.label, role: choice.requiresStrongEmphasis ? .diagnostic : .body,
                               attributes: index == focusedChoiceIndex ? .bold : [])
            )
        })
        let children = choices.enumerated().map { index, choice in
            SemanticNode(id: SemanticID(rawValue: "permission-choice-\(index)"), role: .button,
                         label: choice.label, state: index == focusedChoiceIndex ? .focused : [], actions: [.activate])
        }
        return AgentGrid.output(id: "permission-prompt", role: .dialog, label: requestedAction,
                                lines: lines, context: context, state: [.modal, .focused], actions: [.focus],
                                children: children)
    }
}
...
extension QuestionPrompt: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        for (questionIndex, question) in questions.enumerated() {
            ...
            case .singleSelection, .multipleSelection:
                ...
                for option in question.options {
                    let marker = question.kind == .singleSelection
                        ? (selected.contains(option.id) ? "●" : "○")
                        : (selected.contains(option.id) ? "☑" : "☐")
                    lines.append(AgentGrid.line(AgentGrid.span("  \(marker) ", role: .listMarker),
                                                AgentGrid.span(option.label),
                                                AgentGrid.span(option.detail.map { "  \($0)" } ?? "", role: .muted)))
                    children.append(SemanticNode(id: SemanticID(rawValue: "question-\(questionIndex)-\(option.id)"),
                                                 role: .listItem, label: option.label,
                                                 state: selected.contains(option.id) ? .selected : [], actions: [.activate]))
                }
            }
        ...
        return AgentGrid.output(id: "question-prompt", role: .dialog, label: "Questions", lines: lines,
                                context: context, state: .modal, actions: [.focus, .submit, .dismiss, .setValue],
                                children: children)
    }
}
...
extension TodoItem: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        let symbol: String = switch state {
        case .pending: "○"
        case .inProgress: context.reduceMotion ? "…" : "◉"
        case .completed: "✓"
        case .cancelled: "−"
        }
        ...
        return AgentGrid.output(id: "todo-item", role: .listItem, label: title, lines: lines,
                                context: context, state: semanticState)
    }
}
...
extension SessionSidebar: AgentComponentRenderable where Item: AgentContentPresentable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        guard isVisible(forTerminalWidth: context.width) else {
            return AgentGrid.output(id: "session-sidebar", role: .group, label: "Session sidebar",
                                    lines: [], context: context)
        }
        ...
        for (sectionIndex, section) in sections.enumerated() {
            ...
            children.append(SemanticNode(id: SemanticID(rawValue: "sidebar-section-\(sectionIndex)"),
                                         role: .group, label: section.title))
        }
        let localContext = AgentRenderContext(width: sidebarWidth, theme: context.theme, elapsed: context.elapsed,
                                              reduceMotion: context.reduceMotion)
        var state: SemanticState = []
        if policy.placement(forTerminalWidth: context.width) == .trailingOverlay { state.insert(.modal) }
        return AgentGrid.output(id: "session-sidebar", role: .group, label: "Session sidebar",
                                lines: lines, context: localContext, state: state,
                                actions: state.contains(.modal) ? [.dismiss] : [], children: children)
    }
}
...
extension AgentStatusFooter: AgentComponentRenderable {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        ...
        let children = visible.enumerated().map { index, field in
            SemanticNode(id: SemanticID(rawValue: "status-field-\(index)"), role: .status,
                         label: field.kind.rawValue, value: field.text)
        }
        return AgentGrid.output(id: "agent-status", role: .status, label: text,
                                lines: [StyledText(text, role: .muted)], context: context, children: children)
    }
}

extension BackgroundPulse: AgentComponentRenderable where Frame == BackgroundPulseFrame {
    public func render(in context: AgentRenderContext) -> AgentRenderOutput {
        let phase = configuration.frameIndex(at: context.reduceMotion ? .zero : context.elapsed)
        ...
        var state: SemanticState = []
        if configuration.requiresAnimationFrames && context.reduceMotion == false { state.insert(.busy) }
        return AgentGrid.output(id: "background-pulse", role: .progressIndicator, label: "Background pulse",
                                lines: [line], context: context, state: state)
    }
}
