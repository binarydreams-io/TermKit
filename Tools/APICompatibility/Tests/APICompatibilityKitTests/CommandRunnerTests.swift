...
struct CommandRunnerTests {
    ...
    func runCompileContractsCommand() throws {
        ...
        #expect(result.standardError.isEmpty)
        let compilerArguments = try #require(
            try? String(contentsOf: setup.argumentLog, encoding: .utf8),
            "Compiler argument log is missing. Runner stderr: \(result.standardError)"
        )
            .split(whereSeparator: \.isNewline)
            .map(String.init)
